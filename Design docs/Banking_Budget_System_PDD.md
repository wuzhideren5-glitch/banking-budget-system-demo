...
- 数据科目与产品科目的绑定方式由”单产品/适用所有产品”二选一扩展为**多产品范围表达**，新增 `product_codes` 字段承载”全部产品 / 公司级空置 / 指定多产品”三类语义。数据科目维护页新增分层多选产品弹窗（`ProductMultiSelectDialog`），支持父级展开到叶子节点。
- Excel 导入交互完成一轮改造：通用导入弹窗支持”新增/更新（upsert）”与”覆盖（replace）”两种模式，预算基础数据导入结果文件明确区分新增、覆盖与失败单元格，提升批量维护可控性与回溯性。
- 废弃 `data_account.applies_to_all_products` 布尔字段，统一由 `product_codes` 表达产品范围：`NULL` 或空字符串 = 适用所有产品，逗号分隔代码串 = 指定多产品，详见 Database PDD §0.2。
- 产品科目编码校验从固定 `Z + 4 位数字` 放宽为 `Z + 4~8 位数字`，兼容层级产品编码（如 Z0101、Z02010304）。
- 同步修复了导入反馈与前端选择交互的若干问题，重点改善产品范围回显、导入结果提示与用户操作闭环。
...
- **数据同步管理**作为费用数据入口总控台，支持同步费用整体框架与部门费用执行数据。
- 本次合并保留原“预算基础数据维护”和“预算预测驱动”入口，同时新增费用相关入口，避免覆盖预算预测驱动模块的现有逻辑。

> 本轮合并仅纳入费用模块相关增量，未采用来源包中会回退现有数据科目范围、产品层级与预算预测驱动逻辑的旧实现。

## 0.9 本轮需求变更（2026-05-09）— merge-release

- 合入 ZLC 提交的“工作台和参数模板”增量，在“预算数据输入”下新增“预测预算工作台”入口，用于集中展示开鑫贷/小小账户预测行、数据科目、假设参数与模板绑定概览。
- 新增“预算基本假设 / 参数与模板维护”入口，支持维护预测参数目录、参数值、预置预测模板配置，以及查看参数被模板或数据科目模板绑定引用的影响关系。
- 后端新增预测工作台概览接口与预算假设接口，前端新增对应页面与 DTO，权限口径按数据录入用户及以上开放。
- 本轮采用增量合并方式保留当前主干中的费用模块、智能报告、预算预测驱动、数据科目指标树等既有能力，未用来源包旧版本覆盖当前主线。

## 0.10 本轮需求变更（2026-05-09）— Codex

- 固化数据科目编码分工：`data_acct_code` 保持系统技术主键和公式/报告/预算明细引用键；完整业务层级编码由 `data_account_metric_binding.binding_code` 承载。
- 数据科目维护页当前采用“指标树节点 + 产品范围 + 数据科目明细”的维护方式；新增数据科目时用户选择指标节点和产品，系统自动生成完整业务编码与技术科目代码。
- 预算预测驱动模块改为读取数据科目指标树绑定作为分类来源，底层输入类科目可维护预算预测值，公式类科目由重算链路按产品维度展示计算结果。
- 报告科目与数据科目的绑定继续以 `report_data_mapping.data_acct_code` 为准；报告侧业务层级通过数据科目绑定表联查，不在报告映射表中重复维护指标树或产品关系。
...
- 数据科目维护与预算预测驱动统一采用“指标树 + 产品范围 + 数据科目明细”的层级体验：用户先感知指标口径树，再在同一指标口径下查看各产品的数据科目；产品不是重复的树层，而是明细行的范围属性。
- 业务层级编码采用“指标节点数字分段编码 + 产品范围编码”的方式表达完整路径，例如 `01.01.31.Z01010101`；其中产品级编码必须直接来自 `product_type.product_code`，公司级使用保留范围 `CORP`。
- `data_account.data_acct_code` 继续作为底层技术主键、公式引用键、报告映射键和预算明细引用键；界面可优先展示业务层级编码，但不得绕开现有 `budget_data`、公式引擎与导入模板对 `data_acct_code` 的依赖。
- 新增数据科目时，用户只选择指标节点与产品范围，技术科目代码由后端自动生成，完整业务编码由系统按规则生成。
...
- **预算数据输入**：**预算基础数据维护** — 在**当前产品**上下文中，以**报告层级 + 数据科目 + 月度列**（12 个月）为主的类表格填报（Figma）；**界面「预算值 / 实际值」切换**对应持久化字段 **`budget_actual`**（**`0`**=预算，**`1`**=实际，见 Database PDD）。页面标题需显示**预算年份 + 当前编辑版本号/版本名称**。**报告层级列**为映射/展开后的**视图**，非 `budget_data` 物理列；落库粒度为 **数据科目 × 产品 × 期间 × 版本 × `budget_actual`**（`budget_data.product_code` 与当前所选产品一致；适用多个产品的数据科目仍按产品分行存储）。批量 Excel 导入须满足 Rules **E.5**（导入幂等）。
...
- **预算填报视图 vs 物理行**：**预算基础数据维护**表格中的**报告科目层级**来自 **`report_account` + `report_data_mapping`** 等对数据科目的展开展示；**物理表 `budget_data`** 存储 **`data_acct_code`、`product_code`、`period_id`、`version_id`、`budget_actual`、`value`** 等，**不存报告树列**。界面切换**当前产品**时，展示 **「适用所有产品」** 或 **`product_code` 与该一致**的 `data_account`，以及对应 **`budget_data.product_code`** 的明细行。
...
- **公式保存触发**：数据科目维护页在预算式/实际式保存成功后，须立即对对应口径触发重算；若目标科目绑定多个产品范围，则按产品清单逐一执行同一公式（每个产品使用自身上下文数据）并回写 `budget_data`。
- **公式引用约束**：公式引用仍使用 `data_acct_code` 作为稳定指针；保存与重算时必须校验引用科目适用于当前产品范围，计算读取限定在当前产品上下文。
...
- **预算输入导入交互规范（MUST）**：预算基础数据维护页采用「下载模板 → 上传预览 → 开始导入并下载结果文件」的弹窗式流程；模板文件为 **`download_template/budget_data_temp.xlsx`**。导入完成后必须返回并下载**在原上传文件上回写评估结果**的工作簿（含颜色与失败原因），避免仅在页面瞬时展示。
...
| 界面元素 | 持久化 |
|----------|--------|
| 科目代码 / 名称 | **`data_account.data_acct_code`**、**`data_acct_name`**；新增时科目代码由后端自动生成，用户不手工填写 |
| 完整业务编码 | **`data_account_metric_binding.binding_code`**，由指标节点编码与产品范围编码自动拼接 |
| 指标树归属 | **`data_account_metric_node`**；界面主交互为“选择指标树节点”，而非手工输入平面分组字段 |
| 产品范围 | **`data_account_metric_binding.scope_code` / `product_code`** → **`product_type`**；产品中文名从产品科目维护表读取 |
| 预算式 / 实际式 | **`budget_formula`**、**`actual_formula`** |
| 公式编辑器左侧科目树（防错） | 按当前科目产品范围过滤可引用科目；保存时后端重复校验，禁止绕过前端规则 |
| **数值类型**（金额/百分比/户数） | **`value_type`**（与 §2.2.1 小数位表对应） |
| 备注 | **`remark`** |

**层级体验补充**

- 数据科目维护页应优先展示“指标树路径 -> 数据科目明细行”的结构，同一指标口径下按产品范围列出明细；不同产品不再额外建立一层树，避免“指标树 + 产品树”双重嵌套造成冗余。
- 用户不直接手输完整业务层级编码；系统根据所选指标节点与产品科目自动生成类似 `01.01.31.Z01010101` 的业务层级编码，并自动继承上级路径。
- 新增数据科目时，用户先选择所属指标节点，再选择产品范围；系统生成 `binding_code` 和 `data_acct_code`，并保证有效绑定不重复。
- 产品末级展示必须与产品科目维护页保持一致，产品名称、层级和编码以 `product_type` 为单一事实来源。

### 4.2.1 预算预测驱动对数据科目层级的使用

- 预算预测驱动不再维护独立的平面驱动分类体系；页面分类树来自 `data_account_metric_node`，可输入行来自 `data_account_metric_binding + data_account`。
- 仅底层输入类数据科目允许人工维护预算预测值；判断口径以数据科目是否存在预算/实际公式为准，公式类科目由后端重算后按产品维度展示。
...
- Excel 导入导出模板应包含完整业务编码、技术科目代码、数据科目名称、产品编码、产品名称、数值类型与月度值，导入时优先按完整业务编码定位，必要时回退校验 `data_acct_code + product_code`。
...
### 4.4 预算基础数据维护（`BudgetInputContent`）

| 界面元素 | 持久化说明 |
|----------|------------|
| **预算值 / 实际值** 切换 | 写入/查询 **`budget_data.budget_actual`**：`0` / `1` |
| 当前产品选择 | 通过 **`data_account_metric_binding`** 与兼容字段筛选适用于当前产品的数据科目；写入/更新明细时带 **`budget_data.product_code`**（与当前产品一致） |
...
| **预算基础数据维护** — **预算值 / 实际值** | `budget_data.budget_actual`（预聚合行见 `budget_summary.budget_actual`） | **`0`** = 预算口径，**`1`** = 实际口径 |
