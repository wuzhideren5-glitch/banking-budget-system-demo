...
- **部门科目维护增强**：扩展部门科目维护能力，以支撑费用框架所需的部门-产品映射与多层级部门路径展示。
...
1. **依赖变更**：`backend/requirements.txt` 新增 `xlrd==2.0.1`，用于读取 `.xls` 格式的费用执行源文件。接收方需执行 `pip install -r backend/requirements.txt`。
2. **源文件约定**：费用同步管理默认读取桌面路径 `~/Desktop/预算系统/费用整体框架.xlsx` 和 `~/Desktop/预算系统/部门费用执行.xls`。如需变更源文件位置，需同步修改后端 Router 中的 `_source_file()` 函数。
3. **数据库迁移**：本次新增表通过 `backend/app/init_db.py` 中的 `ensure_databases()` 自动创建，无需手动迁移脚本。已有数据库在下次启动时会自动补齐新表。
...
5. **旧模块保留说明**：本次改动以"新增模块"为主，未删除旧模块（数据科目维护、报告科目维护、产品科目维护、旧预算录入、透视图表、Agent 等）的代码，仅通过导航树隐藏了入口。如需彻底精简，可参考后续精简方案执行。
