    ...
    *,
    ...
    def _source_candidates(name: str) -> list[Path]:
        return [
            ...
            _desktop_budget_dir() / name,
        ]

    def _try_source_file(names: list[str]) -> Path | None:
        for name in names:
            for path in _source_candidates(name):
                if path.exists():
                    return path
        return None

    def _source_file_any(names: list[str]) -> Path:
        path = _try_source_file(names)
        if path is not None:
            return path
        candidates: list[Path] = []
        for name in names:
            candidates.extend(_source_candidates(name))
        raise HTTPException(status_code=400, detail=f"缺少源文件：{' 或 '.join(str(p) for p in candidates)}")

    def _source_file(name: str) -> Path:
        return _source_file_any([name])
    ...
    def _parse_framework_source() -> ParsedFramework:
        ...
        if product_sheet_name:
            ...
            ws_product = wb[product_sheet_name]
    ...
    def _parse_framework_source_bytes(source_file: str, raw: bytes) -> ParsedFramework:
        source_name = _text(source_file).lower()
        is_xls = source_name.endswith(".xls")
        if is_xls:
            wb = xlrd.open_workbook(file_contents=raw)
            if "预算部门" not in wb.sheet_names() or "部门预算科目" not in wb.sheet_names():
                raise HTTPException(status_code=400, detail="上传文件缺少“预算部门”或“部门预算科目”工作表")
        else:
        ...
        for row_idx in _sheet_rows(ws_budget):
            entity_name = _text(_cell_value(ws_budget, row_idx, 0))
            group_name = _text(_cell_value(ws_budget, row_idx, 1))
            owner_name = _text(_cell_value(ws_budget, row_idx, 2))
            row = FrameworkBudgetDepartmentRow(
                entity_name=entity_name,
                group_name=group_name,
                owner_name=owner_name,
                budget_department=_normalize_leaf_department_name(
                    ...
                    group_name,
                    owner_name,
                    _text(_cell_value(ws_budget, row_idx, 3)),
                ),
            )
            key = (row.entity_name, row.group_name, row.owner_name, row.budget_department)
            if not all(key) or key in seen_budget:
        ...
        if ws_product is not None:
            ...
            for row_idx in _sheet_rows(ws_product):
                entity_name = _text(_cell_value(ws_product, row_idx, 0))
                group_name = _text(_cell_value(ws_product, row_idx, 1))
                owner_name = _text(_cell_value(ws_product, row_idx, 2))
                row = FrameworkProductDepartmentRow(
                    entity_name=entity_name,
                    group_name=group_name,
                    owner_name=owner_name,
                    product_department=_normalize_leaf_department_name(
                        ...
                        group_name,
                        owner_name,
                        _text(_cell_value(ws_product, row_idx, 3)),
                    ),
                )
                key = (row.entity_name, row.group_name, row.owner_name, row.product_department)
                if not all(key) or key in seen_product:
        ...
        for row_idx in _sheet_rows(ws_subject):
            subject_name = _text(_cell_value(ws_subject, row_idx, 1))
            if not subject_name or subject_name in seen_subject:
                continue
            seen_subject.add(subject_name)
            subjects.append(
                FrameworkSubjectRow(
                    level_label=_text(_cell_value(ws_subject, row_idx, 0)),
                    budget_subject=subject_name,
                    manage_department=_text(_cell_value(ws_subject, row_idx, 2)),
                    formula_text=_text(_cell_value(ws_subject, row_idx, 3)),
                    sort_order=len(subjects) + 1,
                )
            )
    ...
    async def _load_framework_context() -> tuple[FrameworkContext, str, str]:
        ...
        meta = await _read_sync_meta()
        has_entity_info = parsed is not None and (
            any(row.entity_name for row in parsed.product_departments)
            or any(row.entity_name for row in parsed.budget_departments)
        )
        ...
        raise HTTPException(
            status_code=400,
            detail="费用整体框架尚未初始化到数据库，请在“数据同步管理”先执行“同步框架”。",
        )
    ...
    def _load_actual_rows_from_source(
        ...
        path = _source_file_any(["部门费用执行.xls", "部门费用执行.xlsx"])
        ...
        if path.suffix.lower() == ".xls":
            ...
            if not wb.sheet_names():
                raise HTTPException(status_code=400, detail=f"{path.name} 缺少工作表")
            ws = wb.sheet_by_name("费用执行表") if "费用执行表" in wb.sheet_names() else wb.sheet_by_index(0)
            if ws.nrows < 2:
                raise HTTPException(status_code=400, detail=f"{path.name} 缺少表头")
            ...
            if missing:
                raise HTTPException(status_code=400, detail=f"{path.name} 缺少字段：{'、'.join(missing)}")
            ...
            header_idx = {name: idx for idx, name in enumerate(headers, start=1) if name}
            missing = [h for h in required_headers if h not in header_idx]
            if missing:
                raise HTTPException(status_code=400, detail=f"{path.name} 缺少字段：{'、'.join(missing)}")

            for row_idx in range(3, ws.max_row + 1):
                owner_name = _canonical_owner_name(_text(ws.cell(row_idx, header_idx["费用归属部门"]).value), ctx)
                month_idx = _parse_month(ws.cell(row_idx, header_idx["期间"]).value)
                amount = _to_float(ws.cell(row_idx, header_idx["金额"]).value)
                budget_subject = _canonical_subject(
                    _text(ws.cell(row_idx, header_idx["科目描述"]).value),
                    ctx,
                )
                if not owner_name or month_idx is None or not budget_subject:
                    continue
    ...
    def _load_actual_rows_from_bytes(
        ctx: FrameworkContext,
        raw: bytes,
        file_name: str,
    ) -> tuple[
        ...
        required_headers = ["费用归属部门", "期间", "金额", "科目描述"]
        raw_name = _text(file_name)
        ext = raw_name.lower().rsplit(".", 1)[-1] if "." in raw_name else "xlsx"
        ...
        if ext == "xls":
            wb = xlrd.open_workbook(file_contents=raw)
            if not wb.sheet_names():
                raise HTTPException(status_code=400, detail=f"{raw_name} 缺少工作表")
            ws = wb.sheet_by_name("费用执行表") if "费用执行表" in wb.sheet_names() else wb.sheet_by_index(0)
            if ws.nrows < 2:
                raise HTTPException(status_code=400, detail=f"{raw_name} 缺少表头")

            headers = [_text(v) for v in ws.row_values(1)]
            header_idx = {name: idx for idx, name in enumerate(headers) if name}
            missing = [h for h in required_headers if h not in header_idx]
            if missing:
                raise HTTPException(status_code=400, detail=f"{raw_name} 缺少字段：{'、'.join(missing)}")

            for row_idx in range(2, ws.nrows):
                owner_name = _canonical_owner_name(_text(ws.cell_value(row_idx, header_idx["费用归属部门"])), ctx)
                month_idx = _parse_month(ws.cell_value(row_idx, header_idx["期间"]))
                amount = _to_float(ws.cell_value(row_idx, header_idx["金额"]))
                budget_subject = _canonical_subject(_text(ws.cell_value(row_idx, header_idx["科目描述"])), ctx)
                if not owner_name or month_idx is None or not budget_subject:
                    continue
                detail_rows += 1
                entity_name = ctx.owner_to_entity.get(owner_name, _default_entity_name())
            ...
            for row_idx in range(3, ws.max_row + 1):
                ...
                month_idx = _parse_month(ws.cell(row_idx, header_idx["期间"]).value)
                amount = _to_float(ws.cell(row_idx, header_idx["金额"]).value)
                budget_subject = _canonical_subject(_text(ws.cell(row_idx, header_idx["科目描述"]).value), ctx)
                if not owner_name or month_idx is None or not budget_subject:
                    continue
                detail_rows += 1
                entity_name = ctx.owner_to_entity.get(owner_name, _default_entity_name())
        ...
        monthly_rows = sum(1 for values in actual_by_owner.values() for value in values if value != 0)
        return actual_by_entity, actual_by_group, actual_by_owner, detail_rows, monthly_rows, raw_name
    ...
    async def _load_actual_rows(
        ...
        if db_rows:
            ...
            import_meta = meta.get("actual_import", {})
            source_desc = (
                f"系统内部表（最近同步自 {import_meta.get('source_file', '费用执行明细导入')}"
                f"，同步时间 {import_meta.get('synced_at', '-')})"
            )
            return actual_by_entity, actual_by_group, actual_by_owner, "internal", source_desc
        ...
        raise HTTPException(
            status_code=400,
            detail="费用执行实际尚未初始化到数据库，请在“数据同步管理”上传“部门费用执行.xls/xlsx”后同步。",
        )
    ...
    async def _load_budget_subject_catalog_rows() -> list[dict[str, Any]]:
        ...
        parsed = await _load_framework_from_db()
        if parsed is None:
            raise HTTPException(status_code=400, detail="费用预算科目目录尚未初始化，请先同步费用整体框架到数据库。")
        return [
            {
                "id": idx,
                "parent_id": None,
    ...
    async def _import_actual_snapshot(
        ctx: FrameworkContext,
        *,
        file_name: str,
        raw: bytes,
    ) -> dict[str, Any]:
        _actual_by_entity, _actual_by_group, actual_by_owner, detail_rows, monthly_rows, source_file = _load_actual_rows_from_bytes(
            ctx=ctx,
            raw=raw,
            file_name=file_name,
        )
    ...
    async def _framework_preview(parsed: ParsedFramework) -> dict[str, Any]:
        parsed = await _merge_framework_with_existing(parsed)
    ...
    async def _sync_framework(parsed: ParsedFramework, body: ExpenseFrameworkSyncRequest) -> dict[str, Any]:
        parsed = await _merge_framework_with_existing(parsed)
    ...
    async def _resolve_report(
        *,
        ...
        if actual_source_mode == "source":
            note_parts.append("当前未检测到已导入的费用执行明细，请先上传“部门费用执行.xls/部门费用执行.xlsx”并执行“同步费用执行”。")
        return {
            "perspective": perspective,
    ...
    async def preview_expense_framework_sync(file: UploadFile = File(...)):
        ...
        return await _framework_preview(parsed)
    ...
    async def sync_expense_framework(
        ...
        file_name, raw = await _read_upload_file(file)
        parsed = _parse_framework_source_bytes(file_name, raw)
        return await _sync_framework(
            parsed,
            ExpenseFrameworkSyncRequest(apply_to_master_data=apply_to_master_data),
        )

    @router.post("/api/expense-budget-execution/admin/actual-sync")
    async def sync_expense_actual(file: UploadFile = File(...)):
        ctx, _framework_source_mode, _framework_source_file = await _load_framework_context()
        file_name, raw = await _read_upload_file(file)
        return await _import_actual_snapshot(ctx, file_name=file_name, raw=raw)
