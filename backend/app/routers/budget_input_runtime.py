    ...
    *,
    ...
    async def load_budget_input(
        ...
        async with aiosqlite.connect(common_path) as cdb, aiosqlite.connect(budget_path) as bdb:
            ...
            # 构建产品层级匹配列表：当前产品及其所有父级
            # 例如 Z01010102 -> [Z01010102, Z010101, Z0101, Z01]
            product_hierarchy = [normalized_product_code]
            code = normalized_product_code
            while len(code) > 3:  # Z01 是3位，最短的有效产品代码
                code = code[:-2]  # 去掉最后两位
                if len(code) >= 3:
                    product_hierarchy.append(code)

            cur_binding_count = await cdb.execute(
                "SELECT COUNT(*) FROM data_account_metric_binding WHERE is_active = 1"
            )
            active_binding_count = int((await cur_binding_count.fetchone())[0] or 0)

            if active_binding_count:
                product_placeholders = ",".join(["?"] * len(product_hierarchy))
                cur_accounts = await cdb.execute(
                    ...
                    SELECT DISTINCT
                           ...
                           d.budget_formula,
                           d.actual_formula,
                           MIN(b.sort_order) AS binding_sort_order
                    FROM data_account_metric_binding b
                    JOIN data_account d ON d.data_acct_code = b.data_acct_code
                    WHERE b.is_active = 1
                      AND (
                        b.scope_code = 'CORP'
                        OR UPPER(COALESCE(b.product_code, '')) IN ({product_placeholders})
                        OR UPPER(b.scope_code) IN ({product_placeholders})
                      )
                    GROUP BY d.data_acct_code, d.data_acct_name, d.value_type, d.budget_formula, d.actual_formula
                    ORDER BY binding_sort_order, d.data_acct_code
                    """,
                    (*product_hierarchy, *product_hierarchy),
                )
            else:
                # Historical databases before metric bindings still rely on product_codes.
                hierarchy_conditions = " OR ".join(
                    "',' || product_codes || ',' LIKE '%,' || ? || ',%'"
                    for _ in product_hierarchy
                )
                cur_accounts = await cdb.execute(
                    f"""
                    SELECT DISTINCT data_acct_code, data_acct_name, value_type, budget_formula, actual_formula
                    FROM data_account
                    WHERE (product_codes IS NULL OR product_codes = 'all' OR UPPER(product_codes) = 'ALL')
                       OR ',' || product_codes || ',' LIKE '%,' || ? || ',%'
                       OR ({hierarchy_conditions})
                    ORDER BY data_acct_code
                    """,
                    (normalized_product_code, *product_hierarchy),
                )
            ...
            if data_codes:
                ...
                if report_codes:
                    ...
                    for rr in await cur_reports.fetchall():
                        ...
                        report_level_map[str(rr[0])] = int(rr[3] or 0)
                    ...
                    while unresolved:
                        ...
                        for rr in fetched:
                            ...
                            report_level_map[code] = int(rr[3] or 0)
            ...
            for data_code in data_codes:
                ...
                report_codes = sorted(
                    set(report_codes_by_data.get(data_code, [])),
                    key=lambda code: (-report_level_map.get(code, 0), code),
                )
                if not report_codes:
                    rows.append(
                        BudgetInputRow(
