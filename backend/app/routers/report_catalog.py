...
def build_report_catalog_router(
    *,
    ...
    async def refresh_report_leaf_flags(db: aiosqlite.Connection) -> None:
        await db.execute(
            """
    ...
    async def apply_report_account_import(
        ...
        """执行报告科目导入（mode=upsert=新增/更新混合，或 mode=replace=清空后全量覆盖）"""
        try:
        ...
        async with aiosqlite.connect(path) as db:
            ...
            # replace 模式：先清空旧数据
            if mode == "replace":
                await db.execute("DELETE FROM report_data_mapping")
                await db.execute("DELETE FROM report_account")
                await db.commit()
            ...
            await refresh_report_leaf_flags(db)
            await db.commit()

            # 写操作日志
            await write_operation_log(
                action_type="IMPORT",
                action_desc=f"报告科目批量导入 {mode}，成功{success_count}条/覆盖{overwrite_count}条/失败{failed_count}条",
                target_table="report_account",
                affected_rows=success_count + overwrite_count,
                before_data={"mode": mode},
                after_data={"success": success_count, "overwrite": overwrite_count, "failed": failed_count},
            )
    ...
    async def list_products():
        ...
        async with aiosqlite.connect(path) as db:
            await db.execute("PRAGMA foreign_keys = ON")
            cur = await db.execute(
                "SELECT product_code, product_name, parent_code, level, remark FROM product_type ORDER BY product_code"
            )
            rows = await cur.fetchall()
        return [ProductTypeRow(product_code=r[0], product_name=r[1], parent_code=r[2], level=r[3], remark=r[4]) for r in rows]
    ...
    async def create_product(body: ProductTypeCreate):
        ...
        async with aiosqlite.connect(path) as db:
            ...
            await db.execute(
                "INSERT INTO product_type (product_code, product_name, parent_code, level, remark) VALUES (?, ?, ?, ?, ?)",
                (body.product_code, body.product_name, body.parent_code, body.level, body.remark),
            )
            await db.commit()
        await write_operation_log(
            action_type="INSERT",
    ...
    async def update_product(code: str, body: ProductTypeUpdate):
        ...
        async with aiosqlite.connect(path) as db:
            await db.execute("PRAGMA foreign_keys = ON")
            cur = await db.execute(
                "SELECT product_code, product_name, parent_code, level, remark FROM product_type WHERE product_code = ?",
                (code,),
            )
            ...
            if body.parent_code is not None:
                ...
                vals.append(body.parent_code if body.parent_code.strip() else None)
        ...
        return ProductTypeRow(product_code=row[0], product_name=row[1], parent_code=row[2], level=row[3], remark=row[4])
    ...
    async def delete_product(code: str):
        ...
        async with aiosqlite.connect(path) as db:
            ...
            if not before:
                raise HTTPException(status_code=404, detail="产品不存在")
            # Check children referencing this as parent
            cur = await db.execute(
                "SELECT COUNT(*) FROM product_type WHERE parent_code = ?",
                (code,),
            )
            if (await cur.fetchone())[0] > 0:
                raise HTTPException(status_code=409, detail="仍有子级产品引用该产品为父级，无法删除")
            cur = await db.execute(
    ...
    # ============================================================
    # 产品科目导入 API（模板下载 / 预览 / 执行）
    # ============================================================

    @router.get("/api/product-types/template")
    async def download_product_template():
        """下载产品科目导入模板（包含当前库里的所有产品）"""
        path = common_db_path()
        async with aiosqlite.connect(path) as db:
            db.row_factory = aiosqlite.Row
            cur = await db.execute(
                "SELECT product_code, product_name, parent_code, level, remark "
                "FROM product_type ORDER BY level, product_code"
            )
            rows = await cur.fetchall()
        ...
        for ri, row in enumerate(rows, 2):
            fill = LEVEL_FILLS.get(int(row["level"]), LEVEL_FILLS[1])
            vals = [
                row["product_code"],
                row["product_name"],
                row["parent_code"] or "",
        ...
        return StreamingResponse(
            buf,
            media_type="application/vnd.openxmlformats-officedocument.spreadsheetml.sheet",
            headers={"Content-Disposition": "attachment; filename=product_type_import_template.xlsx"},
        )

    @router.post("/api/product-types/import-preview")
    async def preview_product_import(file: UploadFile = File(...)):
        """预览 Excel 导入内容，校验格式并返回预览行"""
        ...
        ws = wb.active
        if ws.title != "产品科目导入" and "产品" not in ws.title:
            raise HTTPException(status_code=400, detail=f"Sheet 名称应为「产品科目导入」，当前为「{ws.title}」")
        ...
        if "code" not in col_map or "name" not in col_map:
            raise HTTPException(status_code=400, detail=f"表头缺少「产品代码」或「产品名称」列。当前表头: {header}")

        preview = []
        errors = []
        for ri in range(2, ws.max_row + 1):
            code = ws.cell(ri, col_map["code"] + 1).value
            name = ws.cell(ri, col_map["name"] + 1).value
            if not code and not name:
                continue
            level_val = None
            if "level" in col_map:
                lv = ws.cell(ri, col_map["level"] + 1).value
                if lv is not None:
                    try:
                        level_val = int(lv)
                    except:
            ...
            if "parent" in col_map:
                pv = ws.cell(ri, col_map["parent"] + 1).value
                parent_val = str(pv).strip() if pv else None
            remark_val = None
            if "remark" in col_map:
                rv = ws.cell(ri, col_map["remark"] + 1).value
                remark_val = str(rv).strip() if rv else None
            ...
            if not code or not name:
                errors.append(f"第{ri}行: 代码或名称为空")

        return {"total": len(preview), "columns": header, "preview_rows": preview[:20], "errors": errors[:10]}

    @router.post("/api/product-types/import-apply")
    async def apply_product_import(file: UploadFile = File(...), mode: str = Form("upsert")):
        """执行产品科目导入（mode=upsert=覆盖同名，或 mode=append=保留旧数据）"""
        contents = await file.read()
        try:
            wb = load_workbook(BytesIO(contents))
        except Exception as e:
        ...
        if "code" not in col_map or "name" not in col_map:
            raise HTTPException(status_code=400, detail=f"表头缺少「产品代码」或「产品名称」列")
        ...
        for ri in range(2, ws.max_row + 1):
            ...
            if "remark" in col_map:
                rv = ws.cell(ri, col_map["remark"] + 1).value
                remark_val = str(rv).strip() if rv else None
            records.append((str(code).strip(), str(name).strip(), level_val, parent_val, remark_val))

        if not records:
            raise HTTPException(status_code=400, detail="Excel 中没有有效的产品数据")

        path = common_db_path()
        async with aiosqlite.connect(path) as db:
            if mode == "replace":
                await db.execute("DELETE FROM product_type")
                inserted = 0
                updated = 0
                for code, name, lvl, parent, remark in records:
                    await db.execute(
                        "INSERT INTO product_type (product_code, product_name, parent_code, level, remark) "
                        "VALUES (?, ?, ?, ?, ?)",
                        (code, name, parent, lvl, remark)
                    )
                    inserted += 1
            else:  # upsert
                inserted = 0
                ...
                for code, name, lvl, parent, remark in records:
                    cur = await db.execute(
                        "SELECT 1 FROM product_type WHERE product_code = ?", (code,)
                    )
                    existing = await cur.fetchone()
                    if existing:
                        await db.execute(
                            "UPDATE product_type SET product_name=?, parent_code=?, level=?, remark=? WHERE product_code=?",
                            (name, parent, lvl, remark, code)
                        )
                        updated += 1
                    else:
                        await db.execute(
            ...
            await write_operation_log(
                action_type="IMPORT",
                action_desc=f"产品科目批量导入 {mode}，新增{inserted}条/更新{updated}条，共{len(records)}条",
                target_table="product_type",
                affected_rows=len(records),
                before_data={"mode": mode, "record_count": len(records)},
                after_data={"inserted": inserted, "updated": updated},
            )

            return {"ok": True, "mode": mode, "inserted": inserted, "updated": updated, "total": len(records)}
