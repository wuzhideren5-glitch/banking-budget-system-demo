...
const HIDDEN_DEPT_ROOT_NAMES = new Set(["历史架构", "虚拟部门"]);
const ENTITY_ORDER = ["微众银行", "科技子", "科技孙"];
...
function filterTree(nodes: TreeNode[], term: string): TreeNode[] {
  ...
  return nodes
    .map((n) => {
      ...
      const match =
        n.dept_code.toLowerCase().includes(s) ||
        n.dept_name.toLowerCase().includes(s) ||
        n.entity_name.toLowerCase().includes(s);
...
function keepLatestStructure(nodes: TreeNode[], depth = 1): TreeNode[] {
  return nodes
    .filter((node) => !(depth === 1 && HIDDEN_DEPT_ROOT_NAMES.has(node.dept_name)))
    .map((node) => ({ ...node, children: keepLatestStructure(node.children, depth + 1) }));
}

function groupTreeByEntity(nodes: TreeNode[]): Array<{ entityName: string; nodes: TreeNode[] }> {
  ...
  const ranked = Array.from(groups.entries()).sort((a, b) => {
    const left = ENTITY_ORDER.indexOf(a[0]);
    const right = ENTITY_ORDER.indexOf(b[0]);
    const leftRank = left === -1 ? Number.MAX_SAFE_INTEGER : left;
    const rightRank = right === -1 ? Number.MAX_SAFE_INTEGER : right;
    return leftRank - rightRank || a[0].localeCompare(b[0], "zh-CN");
  });
  return ranked.map(([entityName, entityNodes]) => ({ entityName, nodes: entityNodes }));
}
...
export function DataDepartmentContent() {
  ...
  const latestTree = useMemo(() => keepLatestStructure(tree), [tree]);
  const visibleTree = useMemo(() => filterTree(latestTree, deptSearch), [latestTree, deptSearch]);
  const entityGroups = useMemo(() => groupTreeByEntity(visibleTree), [visibleTree]);
  const entityNames = useMemo(() => groupTreeByEntity(latestTree).map((group) => group.entityName), [latestTree]);
  ...
  const collapseCurrentLevelOnly = () => {
    ...
    if (selectedEntityName) {
      const key = `entity:${selectedEntityName}`;
  ...
  const expandAllFromCurrentLevel = () => {
    if (selectedEntityName) {
      const target = groupTreeByEntity(latestTree).find((group) => group.entityName === selectedEntityName);
      if (!target) return;
      ...
      target.nodes.forEach(collect);
      setExpanded((prev) => ({ ...prev, ...next }));
      return;
    }
    if (selectedDeptCode) {
      const selectedNode = findTreeNodeByCode(latestTree, selectedDeptCode);
  ...
  const setEntityExpanded = (entityName: string, open: boolean) => {
  ...
  const expandEntityBranch = (entityName: string) => {
    const target = groupTreeByEntity(latestTree).find((group) => group.entityName === entityName);
    if (!target) return;
    const next: Record<string, boolean> = { [`entity:${entityName}`]: true };
    const collect = (node: TreeNode) => {
      next[node.dept_code] = true;
      node.children.forEach(collect);
    };
  ...
  const renderNode = (n: TreeNode, displayLevel: number): JSX.Element => {
    ...
    return (
      <div key={n.dept_code}>
          ...
          mapped.map((m) => {
            ...
            return (
              <div
                ...
                style={{
                  paddingLeft: `${displayLevel * TREE_INDENT + 8}px`,
        ...
        {isOpen && n.children.map((c) => renderNode(c, displayLevel + 1))}
      </div>
    );
  };
          ...
          <div className="flex-1 overflow-auto">
            ...
            {entityGroups.map((group) => (
              <div key={group.entityName}>
                <div
                  className={`flex items-center gap-2 px-2 py-1.5 border-b border-gray-200 bg-slate-50 sticky top-0 z-[1] ${selectedEntityName === group.entityName ? "bg-blue-100" : ""}`}
                  onClick={() => {
                    setSelectedDeptCode(null);
                  ...
                  onContextMenu={(e) => {
                    ...
                    setContextMenu({
                      ...
                      node: {
                        ...
                        isOpen: expanded[`entity:${group.entityName}`] ?? true,
                      },
                    });
                  }}
                >
                  <button className="p-0.5 hover:bg-gray-200 rounded" onClick={() => toggleEntityExpanded(group.entityName)}>
                    {(expanded[`entity:${group.entityName}`] ?? true) ? (
                  ...
                  <span className="w-24 font-mono text-xs text-slate-500">主体</span>
                  <span className="text-xs font-medium text-slate-800">{group.entityName}</span>
                </div>
                {(expanded[`entity:${group.entityName}`] ?? true) &&
                  group.nodes.map((node) => renderNode(node, 2))}
              </div>
            ))}
          </div>
        </div>
        ...
        >
          {contextMenu.node.type === "entity" && (
            <>
              <button
                onClick={() => {
                  toggleEntityExpanded(contextMenu.node.entityName);
                  setContextMenu(null);
                }}
                className="w-full px-3 py-1.5 text-xs text-left hover:bg-gray-100 flex items-center gap-2"
              >
                <Minimize2 className="w-3 h-3" />
                {contextMenu.node.isOpen ? "收起主体" : "展开主体"}
              </button>
              <button
                onClick={() => {
                  setSelectedEntityName(contextMenu.node.entityName);
                  setSelectedDeptCode(null);
                  expandEntityBranch(contextMenu.node.entityName);
                  setContextMenu(null);
                }}
                className="w-full px-3 py-1.5 text-xs text-left hover:bg-gray-100 flex items-center gap-2"
              >
              ...
              <button
                onClick={() => {
                  setSelectedEntityName(contextMenu.node.entityName);
                  setSelectedDeptCode(null);
                  setEntityExpanded(contextMenu.node.entityName, false);
                  setContextMenu(null);
                }}
                className="w-full px-3 py-1.5 text-xs text-left hover:bg-gray-100 flex items-center gap-2"
              >
