...
const monthlyFormatColumns = ["数据科目代码", "产品科目代码", "M1", "M2", "M3", "M4", "M5", "M6"];
const budgetMonthlySample = ["A1001", "Z0001", "120", "130", "135", "", "", ""];
const actualMonthlySample = ["A1001", "Z0001", "118", "126", "140", "", "", ""];
    ...
    <div className="fixed inset-0 bg-black bg-opacity-50 flex items-center justify-center z-50">
      <div className="bg-white rounded-lg shadow-xl w-[90vw] h-[85vh] flex flex-col">
        <div className="flex items-center justify-between px-4 py-3 border-b border-gray-200 bg-gray-50">
          <h3 className="text-sm font-medium text-gray-800">部门费用数据维护 - Excel导入</h3>
        ...
        <div className="flex-1 overflow-auto p-4">
          <div className="mb-4 rounded border border-blue-200 bg-blue-50 p-3">
            <h4 className="text-xs font-medium text-blue-900 mb-2">分月补录格式说明</h4>
            <div className="space-y-2 text-xs text-blue-900">
              <p>建议先按“数据科目代码 + 产品科目代码 + M1-M12”整理月度数据，再复制到系统模板中导入。</p>
              <p>预算数据填入 `预算数据` 工作表，实际数据填入 `实际数据` 工作表；不需要补录的月份留空即可。</p>
            </div>
            <div className="mt-3 grid grid-cols-1 gap-3 xl:grid-cols-2">
              <div className="rounded border border-blue-200 bg-white p-3">
                <h5 className="text-xs font-medium text-gray-800 mb-2">预算补录示例</h5>
                <div className="overflow-auto">
                  <table className="w-full text-xs border-collapse">
                    <thead className="bg-blue-50">
                      <tr>
                        {monthlyFormatColumns.map((col) => (
                          <th key={`budget-${col}`} className="border border-blue-200 px-2 py-1 text-left font-medium text-gray-700 whitespace-nowrap">
                            {col}
                          </th>
                        ))}
                      </tr>
                    ...
                    <tbody>
                      <tr>
                        {budgetMonthlySample.map((value, idx) => (
                          <td key={`budget-sample-${monthlyFormatColumns[idx]}`} className="border border-blue-200 px-2 py-1 text-gray-700 whitespace-nowrap">
                            {value || "-"}
                          </td>
                        ))}
                      </tr>
              ...
              <div className="rounded border border-blue-200 bg-white p-3">
                <h5 className="text-xs font-medium text-gray-800 mb-2">实际补录示例</h5>
                <div className="overflow-auto">
                  <table className="w-full text-xs border-collapse">
                    <thead className="bg-blue-50">
                      <tr>
                        {monthlyFormatColumns.map((col) => (
                          <th key={`actual-${col}`} className="border border-blue-200 px-2 py-1 text-left font-medium text-gray-700 whitespace-nowrap">
                            {col}
                          </th>
                        ))}
                      </tr>
                    ...
                    <tbody>
                      <tr>
                        {actualMonthlySample.map((value, idx) => (
                          <td key={`actual-sample-${monthlyFormatColumns[idx]}`} className="border border-blue-200 px-2 py-1 text-gray-700 whitespace-nowrap">
                            {value || "-"}
                          </td>
                        ))}
                      </tr>
            ...
            <div className="mt-3 text-xs text-blue-900 space-y-1">
              <p>字段说明：`数据科目代码` 和 `产品科目代码` 必填；`M1-M12` 按月份填写数字。</p>
              <p>填写提醒：百分比类科目建议直接填数字，例如 `5` 表示 `5%`；不要在月份单元格中填写文字说明。</p>
            </div>
          </div>
