...
def build_budget_subject_catalog_router(
    *,
    ...
    def _source_file(name: str) -> Path:
        path = Path.home() / "Desktop" / "预算系统" / name
        if not path.exists():
    ...
    async def _ensure_bootstrapped() -> None:
        async with aiosqlite.connect(common_db_path()) as db:
            ...
            if framework_rows:
                ...
                rows_to_insert = _build_bootstrap_rows(_load_framework_rows_from_source())
    ...
    def _load_framework_rows_from_source() -> list[dict[str, Any]]:
        path = _source_file("费用整体框架.xlsx")
        wb = load_workbook(path, data_only=True)
        if "部门预算科目" not in wb.sheetnames:
            raise HTTPException(status_code=400, detail="费用整体框架.xlsx 缺少“部门预算科目”工作表")
        ws = wb["部门预算科目"]
        rows: list[dict[str, Any]] = []
        for row_idx in range(2, ws.max_row + 1):
            level_label = _text(ws.cell(row_idx, 1).value)
            subject_name = _text(ws.cell(row_idx, 2).value)
            formula_text = _text(ws.cell(row_idx, 4).value) or None
            if not level_label or not subject_name:
                continue
            rows.append(
                {
