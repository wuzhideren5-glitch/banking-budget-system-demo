...
async def _recalculate_dirty_budget_data_formulas_for_year(budget_path: Path, budget_year: int) -> int:
    async with aiosqlite.connect(common_db_path()) as cdb:
        ...
        cur = await cdb.execute(
            """
            SELECT data_acct_code, product_code, applies_to_all_products, budget_formula, actual_formula
            FROM data_account
            """
        )
    ...
    for r in rows:
        ...
        formula_meta[code] = {
            ...
            "applies_to_all_products": int(r[2] or 0),
    ...
    for version_id, budget_actual, data_code_raw in dirty_rows:
        ...
        if not formula:
            continue
        if int(meta.get("applies_to_all_products") or 0) == 1:
            recalculated += int(
...
async def _load_data_account_scope_map(db: aiosqlite.Connection) -> dict[str, bool]:
    cur = await db.execute("SELECT data_acct_code, applies_to_all_products FROM data_account")
    rows = await cur.fetchall()
    return {str(r[0]): int(r[1] or 0) == 1 for r in rows}


def _validate_formula_reference_scope(
    *,
    formula: str | None,
    target_applies_to_all_products: bool,
    ...
    if target_applies_to_all_products:
...
async def _recalculate_data_account_formula_all_products(
    ...
    *,
    ...
    async with aiosqlite.connect(common_path) as cdb:
        await cdb.execute("PRAGMA foreign_keys = ON")
        cur = await cdb.execute(
            "SELECT applies_to_all_products, product_code FROM data_account WHERE data_acct_code = ?",
            (code_u,),
        )
        row = await cur.fetchone()
    if not row:
        return 0
    applies_all = int(row[0] or 0)
...
async def _rebuild_budget_summary_for_version(
    ...
    async with aiosqlite.connect(common_path) as cdb, aiosqlite.connect(resolved_budget_path) as bdb:
        ...
        cur_data_accounts = await cdb.execute(
            """
            SELECT data_acct_code, data_acct_name, product_code, applies_to_all_products, value_type
            FROM data_account
            """
        )
        ...
        data_account_map = {
            str(r[0]): {
                ...
                "applies_to_all_products": int(r[3] or 0),
                "value_type": str(r[4]),
            }
...
def _row_to_account(r: tuple[Any, ...]) -> DataAccountRow:
    return DataAccountRow(
        ...
        value_type=r[7],
        remark=r[8],
        product_name=r[9] if len(r) > 9 else None,
...
async def _get_account_row(
    ...
    cur = await db.execute(
        """
        SELECT data_acct_code, data_acct_name, product_code, applies_to_all_products, budget_formula,
               actual_formula, need_calc, value_type, remark
