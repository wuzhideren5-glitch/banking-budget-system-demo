...
export function ExcelUploadDialog({
  ...
  const [selectedMode, setSelectedMode] = useState<"upsert" | "replace">(importMode);
  ...
  const systemFieldDefaults = useMemo(
    () => ({
      ...
      valueType: "数值类型",
      remark: "备注",
      metricBindingCode: "完整层级编码",
      metricPath: "指标路径",
      metricNodeCode: "指标口径编码",
      scopeCode: "产品范围码",
  ...
  const handleFileSelect = async (file: File) => {
    ...
    try {
      ...
      const resp = await fetch(buildApiUrl(previewEndpoint), { method: "POST", credentials: "include", body: form });
      if (!resp.ok) {
  ...
  const handleImport = async () => {
    ...
    try {
      ...
      // Check if response is JSON (product import) or Excel blob (dept/report import)
      const contentType = resp.headers.get("content-type") || "";
      const totalStr = resp.headers.get("X-Import-Total") || "";
      const successStr = resp.headers.get("X-Import-Success") || "";
      const overwriteStr = resp.headers.get("X-Import-Overwrite") || "";
      const failedStr = resp.headers.get("X-Import-Failed") || "";

      if (totalStr) {
        // Excel result file response (dept/report import)
        if (!resp.ok) {
          const errText = await resp.text();
          throw new Error(errText || "导入失败");
        }
        const total = parseInt(totalStr, 10) || 0;
        const success = parseInt(successStr, 10) || 0;
        const overwrite = parseInt(overwriteStr, 10) || 0;
        ...
        const fileName = fnMatch?.[1] || "import_result.xlsx";
        const url = URL.createObjectURL(blob);
        const a = document.createElement("a");
        a.href = url;
        a.download = fileName;
        ...
        const summary = `导入完成：共 ${total} 条，成功 ${success} 条，覆盖 ${overwrite} 条，失败 ${failed} 条。结果文件已下载：${fileName}`;
        setImportSummary(summary);
        alert(summary);
        await onImportComplete?.();
        handleClose();
      } else {
        // JSON response (product import or other)
        if (!resp.ok) {
          throw new Error((await resp.text()) || "导入失败");
        }
        const result = (await resp.json()) as { ok?: boolean; inserted?: number; updated?: number; total?: number; detail?: string };
        ...
        const summary = `导入完成：共 ${total} 条，新增 ${inserted} 条，更新 ${updated} 条。`;
        setImportSummary(summary);
        alert(summary);
        await onImportComplete?.();
        handleClose();
      ...
      <div className="bg-white rounded-lg shadow-xl w-[90vw] h-[85vh] flex flex-col">
        ...
        <div className="px-4 py-3 border-t border-gray-200 bg-gray-50 flex items-center justify-between">
          ...
          <div className="flex items-center gap-2">
            {/* 导入模式选择（仅当 props 允许 replace 时显示） */}
            {importMode === "replace" && (
              <div className="flex items-center gap-2 mr-3 text-xs">
                <span className="text-gray-600">导入模式：</span>
                <label className={`flex items-center gap-1 cursor-pointer px-2 py-1 rounded border ${selectedMode === "upsert" ? "border-blue-500 bg-blue-50 text-blue-700" : "border-gray-300 text-gray-600 hover:bg-gray-50"}`}>
                  <input
                    type="radio"
                    name="importMode"
                    value="upsert"
                    checked={selectedMode === "upsert"}
                    onChange={() => setSelectedMode("upsert")}
                    className="w-3 h-3 accent-blue-600"
                  />
                  新增/更新
                </label>
                <label className={`flex items-center gap-1 cursor-pointer px-2 py-1 rounded border ${selectedMode === "replace" ? "border-red-500 bg-red-50 text-red-700" : "border-gray-300 text-gray-600 hover:bg-gray-50"}`}>
                  <input
                    type="radio"
                    name="importMode"
                    value="replace"
