...
export function ConfigSystemContent() {
  ...
  const fetchDatabases = async (syncFirst = false) => {
    ...
    setLoadError(null);
    try {
      const rows = syncFirst
        ? await apiPost<SystemDatabaseRowDto[]>("/api/system/databases/sync", {})
        : await apiGet<SystemDatabaseRowDto[]>("/api/system/databases");
      setDatabases(rows);
  ...
  const fetchVersions = async (dbId: number | null) => {
    ...
    try {
      const rows = await apiGet<SystemVersionRowDto[]>(`/api/system/databases/${dbId}/versions`);
  ...
  const fetchPeriodYears = async () => {
    try {
      const rows = await apiGet<Array<{ year: number }>>("/api/system/period-years");
  ...
  const fetchEditShow = async () => {
    try {
      const state = await apiGet<EditShowVersionStateDto>("/api/system/edit-show-version");
      setEditState(state);
  ...
  const fetchCompareSyncLatest = async () => {
    try {
      const row = await apiGet<CompareSyncLatestStatusDto>("/api/compare-summary/sync/latest");
  ...
  const persistEditShow = async (silent = false) => {
    ...
    try {
      ...
      const saved = await apiPut<EditShowVersionStateDto>("/api/system/edit-show-version", payload);
      setEditState(saved);
  ...
  useEffect(() => {
    ...
    void (async () => {
      ...
      // 即使配置未变化，离开该界面也强制同步 compare 底层库，避免透视页读到旧缓存。
      await apiPost("/api/compare-summary/sync?trigger_source=auto_after_setting_save", {});
      window.dispatchEvent(new CustomEvent("budget-version-snapshot-changed"));
      await fetchCompareSyncLatest();
    })().catch(() => {
  ...
  const createDatabase = async () => {
    ...
    setCreatingDb(true);
    try {
      await apiPost<SystemDatabaseRowDto>("/api/system/databases", {
        year: Number(newDbYear),
        first_version_name: newDbVersionName.trim(),
      });
  ...
  const deleteDatabase = async () => {
    ...
    setDeletingDb(true);
    try {
      await apiDelete(`/api/system/databases/${selectedDbId}`);
      await fetchDatabases(true);
      await fetchEditShow();
  ...
  const createVersion = async () => {
    ...
    try {
      await apiPost<SystemVersionRowDto>(`/api/system/databases/${selectedDbId}/versions`, {
  ...
  const saveVersionName = async (v: SystemVersionRowDto) => {
    ...
    setLoadError(null);
    try {
      const row = await apiPatch<SystemVersionRowDto>(
        `/api/system/databases/${selectedDbId}/versions/${v.version_id}`,
        { version_name: draft },
      );
      setVersionNameDrafts((prev) => ({
        ...prev,
  ...
  const deleteVersion = async () => {
    ...
    try {
      await apiDelete(`/api/system/databases/${selectedDbId}/versions/${versionId}`);
      await fetchVersions(selectedDbId);
      await fetchEditShow();
      setDeleteVersionId("");
      alert("版本删除成功");
  ...
  return (
    <div className="p-4 h-full overflow-auto">
      <h3 className="text-sm font-medium text-gray-800 mb-3">系统设定控制</h3>
      ...
      <p className="text-[11px] text-gray-600 mb-4">{activeTabMeta.description}</p>

      {loadError && <div className="mb-3 text-xs text-red-600">{loadError}</div>}
      ...
      {activeTab === "database" && (
        <div className="space-y-3">
          <div className="flex items-center gap-2">
            ...
            {loadingDb && <span className="text-xs text-gray-500">正在同步数据库清单...</span>}
          </div>
          <div className="border border-gray-300 rounded p-3">
            <div className="text-xs font-medium text-gray-800 mb-2">新建年度库</div>
            <div className="flex items-center gap-2 flex-wrap">
              <label className="text-xs text-gray-700">年度</label>
              <select
                value={newDbYear}
              ...
              <label className="text-xs text-gray-700">首版本名称</label>
              <input
              ...
              <button
                ...
                className="px-3 py-1.5 text-xs rounded bg-blue-500 text-white hover:bg-blue-600 disabled:opacity-60 ml-auto"
                disabled={creatingDb}
              >
              ...
              <button
                ...
                className="px-3 py-1.5 text-xs rounded bg-red-500 text-white hover:bg-red-600 disabled:opacity-60"
          ...
          <table className="w-full text-xs border-collapse">
            <thead className="bg-gray-100">
              <tr>
                ...
                <th className="border border-gray-300 px-2 py-1 text-left">文件名</th>
                <th className="border border-gray-300 px-2 py-1 text-left">年份</th>
                <th className="border border-gray-300 px-2 py-1 text-left">创建时间</th>
                <th className="border border-gray-300 px-2 py-1 text-left">文件路径</th>
              </tr>
            </thead>
            <tbody>
              {databases.map((db) => (
                ...
                >
                  <td className="border border-gray-300 px-2 py-1">{db.id}</td>
                  <td className="border border-gray-300 px-2 py-1">{db.data_file_name}</td>
                  <td className="border border-gray-300 px-2 py-1">{db.year}</td>
                  <td className="border border-gray-300 px-2 py-1">{formatDateTime(db.create_time)}</td>
                  <td className="border border-gray-300 px-2 py-1">{db.file_path}</td>
                </tr>
              ))}
              {!databases.length && (
                <tr>
                  <td colSpan={5} className="border border-gray-300 px-2 py-3 text-center text-gray-500">
                    暂无数据库记录
                  </td>
                </tr>
              )}
      ...
      {activeTab === "version" && (
        <div className="space-y-3">
          <div className="flex items-center gap-2">
            <label className="text-xs text-gray-700">选择数据库：</label>
            <select
              value={selectedDbId ?? ""}
            ...
            {loadingVersions && <span className="text-xs text-gray-500">正在加载版本...</span>}
          </div>
          <div className="border border-gray-300 rounded p-3">
            <div className="text-xs font-medium text-gray-800 mb-2">新建版本</div>
            <div className="flex items-center gap-2 flex-wrap">
              <label className="text-xs text-gray-700">版本名称</label>
              <input
              ...
              />
              <label className="text-xs text-gray-700">继承父版本</label>
              <select
              ...
              <label className="text-xs text-gray-700">当前月份</label>
              <select
          ...
          <div className="border border-gray-300 rounded p-3">
            <div className="text-xs font-medium text-gray-800 mb-2">删除版本</div>
            <div className="flex items-center gap-2 flex-wrap">
              <label className="text-xs text-gray-700">版本ID</label>
              <select
              ...
              <button
                ...
                className="px-3 py-1.5 text-xs rounded bg-red-500 text-white hover:bg-red-600 disabled:opacity-60 ml-auto"
          ...
          <table className="w-full text-xs border-collapse">
            <thead className="bg-gray-100">
              <tr>
                <th className="border border-gray-300 px-2 py-1 text-left">版本ID</th>
                <th className="border border-gray-300 px-2 py-1 text-left min-w-[12rem]">版本名称（可编辑）</th>
                <th className="border border-gray-300 px-2 py-1 text-left">创建时间</th>
                <th className="border border-gray-300 px-2 py-1 text-left">当前月份</th>
              </tr>
            </thead>
            <tbody>
              {versions.map((v) => (
                <tr key={v.version_id}>
                  <td className="border border-gray-300 px-2 py-1">{v.version_id}</td>
                  <td className="border border-gray-300 px-2 py-1">
                    <div className="flex items-center gap-1">
                      <input
                        type="text"
                        className="border border-gray-300 rounded px-1.5 py-0.5 text-xs flex-1 min-w-0 max-w-[18rem]"
                  ...
                  <td className="border border-gray-300 px-2 py-1">{formatDateTime(v.version_date_time)}</td>
                  <td className="border border-gray-300 px-2 py-1">{v.current_month}</td>
                </tr>
              ))}
              {!versions.length && (
                <tr>
      ...
      {activeTab === "edit_show" && (
        <div className="space-y-4">
          <div className="border border-gray-300 rounded p-3">
            <div className="text-xs font-medium text-gray-800 mb-2">当前可编辑版本（唯一）</div>
            <div className="flex items-center gap-3 flex-wrap">
              <div className="flex items-center gap-1.5">
                ...
                <select
                  ...
                  className="border border-gray-300 rounded px-2 py-1 text-xs min-w-[14rem]"
                >
              ...
              <div className="flex items-center gap-1.5">
                ...
                <select
                  ...
                  className="border border-gray-300 rounded px-2 py-1 text-xs min-w-[16rem]"
            ...
            <div className="flex items-center justify-between gap-2 mb-2">
              ...
              <div className="text-[11px] text-gray-600">
                {compareSyncLatest?.job_id ? (
                  <>
                    最近Compare同步：{(compareSyncLatest.status ?? "unknown").toUpperCase()}{" "}
