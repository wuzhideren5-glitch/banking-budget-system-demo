...
export function BudgetInputContent() {
  ...
  useEffect(() => {
    (async () => {
      try {
        ...
        alert(e instanceof Error ? `初始化预算基础数据维护失败：${e.message}` : "初始化失败");
      }
    })();
  ...
  useEffect(() => {
    void loadBudgetData().catch((e) => alert(e instanceof Error ? `加载预算数据失败：${e.message}` : "加载失败"));
    // eslint-disable-next-line react-hooks/exhaustive-deps
  ...
  const filteredRows = useMemo(() => {
    ...
    return rows.filter((row) => {
      ...
      return keywords.some(
        (kw) => reportText.includes(kw) || dataAcctCode.includes(kw) || dataAcctName.includes(kw)
      );
    });
  }, [rows, searchText]);
  ...
  const tableMinWidth = reportW + dataW + valueTW + periodsWidthSum + totalW;
      ...
      <div className="flex-1 border border-gray-300 rounded overflow-auto" style={{ maxHeight: "calc(100vh - 200px)" }}>
        <table className="text-xs border-collapse" style={{ minWidth: `${Math.max(1200, tableMinWidth)}px` }}>
          ...
          <tbody className="bg-white">
            {loading ? (
              <tr>
                <td colSpan={periods.length + 4} className="px-2 py-6 text-center text-gray-500">
                  正在从数据库加载预算基础数据...
                </td>
              </tr>
