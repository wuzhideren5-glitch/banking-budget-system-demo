    ...
    *,
    ...
    async def load_budget_input(
        ...
        async with aiosqlite.connect(common_path) as cdb, aiosqlite.connect(budget_path) as bdb:
            ...
            # 构建 OR 条件来匹配层级中的任意产品
            # 每个产品代码用 LIKE '%,code,%' 来匹配逗号分隔的 product_codes
            ...
            cur_accounts = await cdb.execute(
                f"""
                SELECT DISTINCT data_acct_code, data_acct_name, value_type, budget_formula, actual_formula, budget_rule_code
                FROM data_account
            ...
            data_rule_map = {str(r[0]): str(r[5] or "").strip() for r in account_rows}
                ...
                if not report_codes:
                    rows.append(
                        BudgetInputRow(
                            ...
                            formula_locked=bool(data_formula_map.get(data_code)) or (budget_actual == 0 and bool(data_rule_map.get(data_code))),
