...
def _path_required_permission(path: str, method: str) -> int | None:
    ...
    if path.startswith("/api/budget-input"):
        return 2
    if path.startswith("/api/forecast-workbench"):
        return 2
...
async def _is_formula_locked_data_account(data_acct_code: str, budget_actual: int) -> bool:
    ...
    async with aiosqlite.connect(path) as db:
        cur = await db.execute(
            """
            SELECT budget_formula, actual_formula, budget_rule_code
            FROM data_account
    ...
    rule_code = str(row[2] or "").strip()
    return bool(formula) or (budget_actual == 0 and bool(rule_code))
...
async def _list_applicable_products_for_account(
    data_acct_code: str,
    *,
    common_path: Path | None = None,
) -> list[str]:
    resolved_common = common_path if common_path is not None else common_db_path()
    async with aiosqlite.connect(resolved_common) as cdb:
        await cdb.execute("PRAGMA foreign_keys = ON")
        cur = await cdb.execute(
        ...
        if not row:
            return []
        product_codes_raw = row[0]
        single_pc = str(row[1] or "").strip().upper()
        if product_codes_raw is None or str(product_codes_raw).upper() == "ALL":
            cur = await cdb.execute("SELECT product_code FROM product_type ORDER BY product_code")
            return [str(r[0]).strip().upper() for r in await cur.fetchall()]
        if str(product_codes_raw).strip() == "":
            return [single_pc] if single_pc else []
        return [part.strip().upper() for part in str(product_codes_raw).split(",") if part.strip()]


async def _load_assumption_value_map(
    *,
    budget_year: int,
    version_id: int,
    scenario_code: str,
    ...
    async with aiosqlite.connect(resolved_common) as cdb:
        ...
        cur = await cdb.execute(
            """
            SELECT parameter_code, month_index, value
            FROM assumption_value
            WHERE budget_year = ?
              AND version_id = ?
              AND scenario_code = ?
              AND product_scope_key = ?
            """,
            (budget_year, version_id, scenario_code, product_scope_key),
        )
        rows = await cur.fetchall()
    return {(str(r[0]).strip().upper(), int(r[1])): float(r[2] or 0.0) for r in rows}


async def _load_assumption_parameter_meta_map(parameter_codes: list[str]) -> dict[str, dict[str, Any]]:
    codes = [code.strip().upper() for code in parameter_codes if code.strip()]
    if not codes:
        return {}
    ...
    async with aiosqlite.connect(common_db_path()) as cdb:
        ...
        cur = await cdb.execute(
            f"""
            SELECT parameter_code, input_mode, value_formula, source_data_code, time_granularity
    ...
    return {
        str(row[0]).strip().upper(): {
            "input_mode": str(row[1] or "manual").strip().lower() or "manual",
            "value_formula": str(row[2] or "").strip(),
            "source_data_code": str(row[3] or "").strip().upper(),
            "time_granularity": str(row[4] or "monthly").strip().lower() or "monthly",
        }
        for row in rows
    }


def _alias_token(prefix: str, raw: str) -> str:
    normalized = re.sub(r"[^A-Z0-9_]", "_", str(raw or "").strip().upper())
    return f"{prefix}_{normalized}" if normalized else prefix


def _materialize_rule_expression(expression: str) -> tuple[str, list[str], list[str], list[str], list[str]]:
    expr = _prepare_formula_expression(expression)
    param_aliases = list(dict.fromkeys(re.findall(r'P\(\s*"([^"]+)"\s*\)', expr)))
    data_aliases = list(dict.fromkeys(re.findall(r'A\(\s*"([^"]+)"\s*\)', expr)))
    prev_actual_refs = list(dict.fromkeys(re.findall(r'PREV_ACTUAL\(\s*"([^"]+)"\s*\)', expr, flags=re.IGNORECASE)))
    ytd_avg_refs = list(dict.fromkeys(re.findall(r'YTD_AVG_ACTUAL\(\s*"([^"]+)"\s*\)', expr, flags=re.IGNORECASE)))
    expr = re.sub(
        r'P\(\s*"([^"]+)"\s*\)',
        lambda m: _alias_token("P", m.group(1)),
        expr,
    )
    expr = re.sub(
        r'A\(\s*"([^"]+)"\s*\)',
        lambda m: _alias_token("A", m.group(1)),
        expr,
    )
    expr = re.sub(
        r'PREV_ACTUAL\(\s*"([^"]+)"\s*\)',
        lambda m: _alias_token("PA", m.group(1)),
        expr,
        flags=re.IGNORECASE,
    )
    expr = re.sub(
        r'YTD_AVG_ACTUAL\(\s*"([^"]+)"\s*\)',
        lambda m: _alias_token("YA", m.group(1)),
        expr,
        flags=re.IGNORECASE,
    )
    expr = re.sub(r"\bDAYS\(\s*\)", "__DAYS__", expr, flags=re.IGNORECASE)
    return expr, param_aliases, data_aliases, prev_actual_refs, ytd_avg_refs


def _evaluate_dynamic_metric_expression(
    expression: str,
    *,
    prev_actual_by_code: dict[str, float],
    ytd_avg_by_code: dict[str, float],
) -> float:
    expr = _prepare_formula_expression(expression)
    if not expr:
        return 0.0
    expr = re.sub(
        r'PREV_ACTUAL\(\s*"([^"]+)"\s*\)',
        lambda m: str(prev_actual_by_code.get(str(m.group(1)).strip().upper(), 0.0)),
        expr,
        flags=re.IGNORECASE,
    )
    expr = re.sub(
        r'YTD_AVG_ACTUAL\(\s*"([^"]+)"\s*\)',
        lambda m: str(ytd_avg_by_code.get(str(m.group(1)).strip().upper(), 0.0)),
        expr,
        flags=re.IGNORECASE,
    )
    return _calculate_formula_value(expr, {})


async def _load_rule_template(rule_code: str) -> tuple[str, dict[str, Any]]:
    async with aiosqlite.connect(common_db_path()) as cdb:
        await cdb.execute("PRAGMA foreign_keys = ON")
        cur = await cdb.execute(
            """
            ...
            WHERE rule_code = ? AND is_enabled = 1
            """,
            (rule_code.strip().upper(),),
        )
        row = await cur.fetchone()
    if not row:
        raise HTTPException(status_code=400, detail=f"预算规则模板不存在：{rule_code}")
    try:
        config = json.loads(str(row[1] or "{}"))
    except json.JSONDecodeError:
        config = {}
    return str(row[0] or "").strip(), config


async def _calculate_formula_rule_values(
    *,
    ...
    period_ids = await _period_ids_for_year(budget_year)
    if not period_ids:
        return {}
    expr, param_aliases, data_aliases, prev_actual_refs, ytd_avg_refs = _materialize_rule_expression(formula_expression)
    if not expr:
        return {}
    period_meta: dict[int, tuple[int, int]] = {}
    async with aiosqlite.connect(common_db_path()) as cdb:
        await cdb.execute("PRAGMA foreign_keys = ON")
        placeholders = ",".join(["?"] * len(period_ids))
        cur = await cdb.execute(
            f"""
            SELECT period_id, month, days
            FROM period
        ...
        for row in await cur.fetchall():
            period_meta[int(row[0])] = (parse_month_index(str(row[1])), int(row[2] or 0))
    parameter_bindings = {
        str(k).strip().upper(): str(v).strip().upper()
        for k, v in dict(merged_config.get("parameter_bindings") or {}).items()
        if str(v or "").strip()
    }
    data_bindings = {
        str(k).strip().upper(): str(v).strip().upper()
        for k, v in dict(merged_config.get("data_bindings") or {}).items()
        if str(v or "").strip()
    }
    product_scope_key = str(merged_config.get("product_scope_key") or product_code.strip().upper())
    scenario_code = str(merged_config.get("scenario_code") or "BASE").strip().upper() or "BASE"
    assumption_values = await _load_assumption_value_map(
        budget_year=budget_year,
        version_id=version_id,
        scenario_code=scenario_code,
        product_scope_key=product_scope_key,
    )
    parameter_codes = sorted({parameter_bindings.get(alias.strip().upper(), alias.strip().upper()) for alias in param_aliases})
    parameter_meta_map = await _load_assumption_parameter_meta_map(parameter_codes)
    ref_data_codes = sorted(
        {
            data_bindings.get(alias.strip().upper(), alias.strip().upper())
            for alias in [*data_aliases, *prev_actual_refs, *ytd_avg_refs]
        }
    )
    ref_value_type_map: dict[str, str] = {}
    if ref_data_codes:
        placeholders = ",".join(["?"] * len(ref_data_codes))
        async with aiosqlite.connect(common_db_path()) as cdb:
            await cdb.execute("PRAGMA foreign_keys = ON")
            cur = await cdb.execute(
                f"""
                ...
                """,
                tuple(ref_data_codes),
            )
            ref_value_type_map = {str(r[0]): str(r[1] or "") for r in await cur.fetchall()}
    data_value_map: dict[tuple[str, int], float] = {}
    actual_value_map: dict[tuple[str, int], float] = {}
    if ref_data_codes:
        async with aiosqlite.connect(budget_path) as bdb:
            await bdb.execute("PRAGMA foreign_keys = ON")
            code_placeholders = ",".join(["?"] * len(ref_data_codes))
            ...
            cur = await bdb.execute(
                ...
                """,
                (version_id, product_code.strip().upper(), *ref_data_codes, *period_ids),
            )
            for row in await cur.fetchall():
                data_value_map[(str(row[0]), int(row[1]))] = float(row[2] or 0.0)
            cur = await bdb.execute(
                f"""
                SELECT data_acct_code, period_id, value
                FROM budget_data
            ...
            for row in await cur.fetchall():
                actual_value_map[(str(row[0]), int(row[1]))] = float(row[2] or 0.0)
    prev_actual_by_code: dict[str, float] = {}
    ytd_avg_by_code: dict[str, float] = {}
    for data_code in ref_data_codes:
        month_values: list[float] = []
        last_value = 0.0
        for period_id in period_ids:
            month_idx, _days = period_meta.get(period_id, (0, 0))
            if month_idx >= current_month:
                continue
            if (data_code, period_id) not in actual_value_map:
                continue
            value = _normalize_formula_ref_value(
                actual_value_map[(data_code, period_id)],
                ref_value_type_map.get(data_code, ""),
            )
            month_values.append(value)
            last_value = value
        prev_actual_by_code[data_code] = last_value
        ytd_avg_by_code[data_code] = (sum(month_values) / len(month_values)) if month_values else 0.0
    result: dict[int, float] = {}
    for period_id in period_ids:
        month_idx, days = period_meta.get(period_id, (0, 0))
        values: dict[str, float] = {"__DAYS__": float(days)}
        for alias in param_aliases:
            alias_u = alias.strip().upper()
            parameter_code = parameter_bindings.get(alias_u, alias_u)
            token = _alias_token("P", alias_u)
            parameter_meta = parameter_meta_map.get(parameter_code, {})
            input_mode = str(parameter_meta.get("input_mode") or "manual").lower()
            if input_mode == "source_prev_actual":
                source_code = str(parameter_meta.get("source_data_code") or "").strip().upper()
                values[token] = float(prev_actual_by_code.get(source_code, 0.0))
            elif input_mode == "source_ytd_avg_actual":
                source_code = str(parameter_meta.get("source_data_code") or "").strip().upper()
                values[token] = float(ytd_avg_by_code.get(source_code, 0.0))
            elif input_mode == "formula":
                values[token] = float(
                    _evaluate_dynamic_metric_expression(
                        str(parameter_meta.get("value_formula") or ""),
                        prev_actual_by_code=prev_actual_by_code,
                        ytd_avg_by_code=ytd_avg_by_code,
                    )
                )
            else:
                month_key = 0 if str(parameter_meta.get("time_granularity") or "").lower() == "annual" else month_idx
                values[token] = float(
                    assumption_values.get((parameter_code, month_key), assumption_values.get((parameter_code, 0), 0.0))
                )
        for alias in data_aliases:
            alias_u = alias.strip().upper()
            data_code = data_bindings.get(alias_u, alias_u)
            token = _alias_token("A", alias_u)
            values[token] = _normalize_formula_ref_value(
                data_value_map.get((data_code, period_id), 0.0),
                ref_value_type_map.get(data_code, ""),
            )
        for alias in prev_actual_refs:
            alias_u = alias.strip().upper()
            data_code = data_bindings.get(alias_u, alias_u)
            values[_alias_token("PA", alias_u)] = float(prev_actual_by_code.get(data_code, 0.0))
        for alias in ytd_avg_refs:
            alias_u = alias.strip().upper()
            data_code = data_bindings.get(alias_u, alias_u)
            values[_alias_token("YA", alias_u)] = float(ytd_avg_by_code.get(data_code, 0.0))
        result[period_id] = _calculate_formula_value(expr, values)
    return result


async def _calculate_budget_rule_values(
    *,
    data_acct_code: str,
    rule_code: str,
    rule_config_json: str | None,
    version_id: int,
    product_code: str,
    budget_path: Path,
    budget_year: int,
    ...
    resolved_common = common_path if common_path is not None else common_db_path()
    rule_type, template_config = await _load_rule_template(rule_code)
    account_config: dict[str, Any] = {}
    try:
        account_config = json.loads(rule_config_json or "{}")
    except json.JSONDecodeError:
        account_config = {}
    merged_config = {**template_config, **account_config}
    if rule_type == "formula_monthly":
        formula_expression = str(merged_config.get("formula_expression") or "").strip()
        return await _calculate_formula_rule_values(
    ...
    scenario_code = str(merged_config.get("scenario_code") or "BASE").strip().upper() or "BASE"
    product_scope_key = str(merged_config.get("product_scope_key") or product_code.strip().upper())
    annual_total_param = str(merged_config.get("annual_total_param") or "").strip().upper()
    use_product_split = bool(
        merged_config.get("product_split_mode") and str(merged_config.get("product_split_mode")) != "none"
    )
    assumption_values = await _load_assumption_value_map(
        budget_year=budget_year,
        version_id=version_id,
        scenario_code=scenario_code,
        product_scope_key=product_scope_key,
        common_path=resolved_common,
    )
    annual_total = float(assumption_values.get((annual_total_param, 0), 0.0))
    if annual_total == 0 and annual_total_param:
        annual_total = float(assumption_values.get((annual_total_param, current_month), 0.0))
    applicable_products = [product_code.strip().upper()]
    if use_product_split:
        applicable_products = await _list_applicable_products_for_account(data_acct_code, common_path=resolved_common)
        if not applicable_products:
            applicable_products = [product_code.strip().upper()]
        annual_total = annual_total / max(len(applicable_products), 1)
    value_by_period_id: dict[int, float] = {}
    ...
    async with aiosqlite.connect(resolved_common) as cdb:
        ...
        for row in await cur.fetchall():
            period_to_month[int(row[0])] = parse_month_index(str(row[1]))
    actual_sum = 0.0
    async with aiosqlite.connect(budget_path) as bdb:
        await bdb.execute("PRAGMA foreign_keys = ON")
        placeholders = ",".join(["?"] * len(period_ids))
        cur = await bdb.execute(
            ...
            """,
            (data_acct_code.strip().upper(), product_code.strip().upper(), version_id, *period_ids),
        )
        for row in await cur.fetchall():
            period_id = int(row[0])
            month_idx = period_to_month.get(period_id, 0)
            if month_idx < current_month:
                actual_sum += float(row[1] or 0.0)
    future_period_ids = [pid for pid in period_ids if period_to_month.get(pid, 0) >= current_month]
    remaining_total = annual_total - actual_sum
    avg_value = remaining_total / len(future_period_ids) if future_period_ids else 0.0
    for pid in future_period_ids:
        value_by_period_id[pid] = avg_value
    return value_by_period_id
...
async def _recalculate_data_account_rule(
    data_acct_code: str,
    rule_code: str,
    rule_config_json: str | None,
    version_id: int,
    ...
    *,
    ...
    resolved_budget_year = budget_year if budget_year is not None else settings.budget_year
    resolved_budget_path = budget_path if budget_path is not None else budget_db_path(resolved_budget_year)
    current_month = await _get_version_current_month(version_id, resolved_budget_path)
    value_by_period_id = await _calculate_budget_rule_values(
    ...
    if not value_by_period_id:
        return 0
    pc_norm = product_code.strip().upper()
    now = _iso_now()
    async with aiosqlite.connect(resolved_budget_path) as db:
        await db.execute("PRAGMA foreign_keys = ON")
        for period_id, value in value_by_period_id.items():
            await db.execute(
                """
                INSERT INTO budget_data (
                  data_acct_code, product_code, period_id, budget_actual, version_id, value, need_calc, create_time, update_time
                ...
                DO UPDATE SET value = excluded.value, need_calc = 0, update_time = excluded.update_time
                """,
                (data_acct_code, pc_norm, period_id, budget_actual, version_id, float(value), now, now),
            )
        await db.commit()
    return len(value_by_period_id)
...
async def _recalculate_data_account_rule_all_products(
    data_acct_code: str,
    rule_code: str,
    rule_config_json: str | None,
    version_id: int,
    ...
    *,
    ...
    common_path = common_db_path()
    code_u = data_acct_code.strip().upper()
    products = await _list_applicable_products_for_account(code_u, common_path=common_path)
    total = 0
    for product_code in products:
        total += await _recalculate_data_account_rule(
...
async def _recalculate_product_formula_rows(
    ...
    *,
    ...
    async with aiosqlite.connect(common_path) as cdb:
        ...
        cur = await cdb.execute(
            f"""
            SELECT data_acct_code, {formula_col}, budget_rule_code, budget_rule_config_json
            FROM data_account
    ...
    rules = [
        (str(r[0]), str(r[2]).strip(), r[3])
        for r in rows
        if budget_actual == 0 and str(r[2] or "").strip()
    ]
    if not formulas and not rules:
        return 0
    recalculated = 0
    ...
    for code, rule_code, rule_config_json in rules:
        recalculated += await _recalculate_data_account_rule(
            data_acct_code=code,
            rule_code=rule_code,
            rule_config_json=rule_config_json,
            version_id=version_id,
...
app.include_router(
    build_data_accounts_router(
        ...
        recalculate_data_account_rule_all_products=(
            lambda **kwargs: _recalculate_data_account_rule_all_products(**kwargs)
        ),
...
def _row_to_account(r: tuple[Any, ...]) -> DataAccountRow:
    # 新字段顺序：data_acct_code, data_acct_name, product_code, product_codes,
    #              budget_formula, actual_formula, need_calc, budget_rule_code, budget_rule_config_json,
    #              value_type, remark, product_name
    product_codes_val = r[3] if len(r) > 3 else None
    return DataAccountRow(
        ...
        budget_rule_code=r[7] if len(r) > 7 else None,
        budget_rule_config_json=r[8] if len(r) > 8 else None,
