...
import { apiGet, type ForecastWorkbenchOverviewDto, type ForecastWorkbenchLineRowDto } from "@/lib/api";
...
export function ForecastWorkbenchContent() {
  ...
  const refresh = async () => {
    ...
    try {
      ...
      setOverview(result);
      if (selectedGroup === "全部" && result.lines.length > 0) {
        return;
      }
        ...
        <div className="grid gap-4 xl:grid-cols-[minmax(0,1.2fr)_minmax(320px,0.8fr)]">
          ...
          <div className="rounded-lg border border-gray-200 bg-white p-4 shadow-sm">
            ...
            <div className="mt-3 space-y-3 text-xs leading-6 text-gray-600">
              <p>该页面已按 `工作簿1.xlsx` 的“开鑫贷 / 小小账户”模板主线预置预测行，先聚焦展示每条预测行绑定了哪些数据科目、参数和模板。</p>
              <p>当前优先覆盖利息净收入、净手续费收入、风险成本、平台费率、渠道费、催收、征信、清结算、保险代偿等关键预测主线。</p>
              <p>后续可继续补三类能力：左侧产品/部门切换、中部月度预测表、右侧公式/模板编辑面板，并把手工锚点逐步替换成真实数据科目或假设参数。</p>
            </div>
              ...
              {selectedLine ? (
                <div className="mt-2 space-y-2 text-xs text-gray-600">
                  ...
                  <div className="space-y-1 pt-1">
                    {selectedLine.bindings.map((item) => (
                      <div key={item.id} className="rounded border border-gray-200 bg-white px-2 py-1.5">
                        <div className="font-medium text-gray-700">
                          {bindingTypeLabel[item.binding_type] || item.binding_type} / {item.binding_name || item.binding_code}
                        </div>
                        <div className="mt-0.5 text-[11px] text-gray-500">
                          编码 {item.binding_code} | 角色 {item.binding_role || "未标注"}
                        </div>
                      </div>
                    ))}
                  </div>
