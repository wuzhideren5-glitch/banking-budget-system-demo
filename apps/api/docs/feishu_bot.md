...
- Python 包：`lark-oapi`（见 `apps/api/requirements.txt`）
...
- `apps/api/app/feishu_bot.py`：长连接、消息处理、调用 `AgentGraphService.chat`
- `apps/api/app/feishu_store.py`：绑定存储与校验
- `apps/api/app/main.py`：`lifespan` 中启动机器人；`/api/system/feishu/bindings` 管理接口
