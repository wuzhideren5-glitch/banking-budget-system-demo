...
async def load_metric_paths_for_data_accounts(
    ...
    *,
    ...
    node_map = {
        ...
        for r in await cur_nodes.fetchall()
    }
    ...
    for row in await cur_bindings.fetchall():
        metric_code = str(row[0] or "").strip().upper()
        data_code = str(row[1] or "").strip().upper()
...
async def load_metric_tree_with_data_accounts(db: aiosqlite.Connection) -> list[dict[str, Any]]:
    cur_nodes = await db.execute(
        """
    ...
    node_rows = await cur_nodes.fetchall()
    children_by_parent: dict[str | None, list[str]] = {}
    sort_order_by_code: dict[str, int] = {}
    nodes: dict[str, dict[str, Any]] = {}
    for code_raw, name_raw, parent_raw, level_raw, node_type_raw, sort_raw in node_rows:
    ...
    cur_bindings = await db.execute(
        ...
        SELECT b.metric_node_code,
               b.data_acct_code,
               COALESCE(d.data_acct_name, ''),
               COALESCE(d.value_type, ''),
               COALESCE(d.budget_formula, ''),
               COALESCE(d.actual_formula, ''),
               COALESCE(b.sort_order, 0),
               COALESCE(b.scope_code, '')
        FROM data_account_metric_binding b
    ...
    for row in await cur_bindings.fetchall():
        metric_code = str(row[0] or "").strip().upper()
        data_code = str(row[1] or "").strip().upper()
        scope_code = str(row[7] or "").strip().upper()
        ...
        parent["children"].append(
            {
                ...
                "budget_formula": str(row[4] or ""),
                "actual_formula": str(row[5] or ""),
