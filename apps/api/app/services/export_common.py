...
def _ascii_fallback_filename(filename: str) -> str:
    ...
    return filename or "workbook.xlsx"
...
def workbook_streaming_response(
    ...
    *,
    ...
    wb.save(out)
    out.seek(0)
    return excel_streaming_response(out, filename=filename, fallback_filename=fallback_filename)
...
def binary_streaming_response(
    ...
    *,
    ...
    if isinstance(content, bytes):
        ...
        out = content
        out.seek(0)
    fallback = fallback_filename or _ascii_fallback_filename(filename)
...
def budget_summary_field_meta() -> dict[str, tuple[str, str]]:
    return {
        "metric_level1": ("指标1级", "metric_level1"),
        "metric_level2": ("指标2级", "metric_level2"),
        "metric_level3": ("指标3级", "metric_level3"),
        "metric_level4": ("指标4级", "metric_level4"),
        "metric_level5": ("指标5级", "metric_level5"),
        "dept_level1": ("部门科目1级", "dept_level1"),
        "dept_level2": ("部门科目2级", "dept_level2"),
        "dept_level3": ("部门科目3级", "dept_level3"),
        "data_code_name": ("机构及产品指标编码", "data_code_name"),
        "product_code_name": ("机构及产品", "product_code_name"),
        "month": ("月份", "month"),
        "quarter": ("季度", "quarter"),
...
def color_worksheet_row_font(ws: Any, row_idx: int, max_col: int, color: str) -> None:
