...
import type { ExcelImportMode, ExcelImportWorkflow } from "@/lib/org-product/excelImportApi";
...
interface ExcelUploadDialogProps {
  ...
  importWorkflow: ExcelImportWorkflow;
...
export function ExcelUploadDialog({
  ...
  const [selectedMode, setSelectedMode] = useState<ExcelImportMode>(importMode);
  ...
  const systemFieldDefaults = useMemo(
    () => ({
      ...
      product: "所属机构及产品代码",
      productCode: "所属机构及产品代码",
      productName: "所属机构及产品名称",
      valueType: "数值类型",
      needCalc: "是否需重计算",
      allowManualEntry: "是否允许手工补录",
      remark: "备注",
      metricPath: "指标路径",
      metricNodeCode: "机构及产品指标编码",
      scopeCode: "产品代码",
      level1Code: "第1级指标节点代码",
      level1Name: "第1级指标节点名称",
      level2Code: "第2级指标节点代码",
      level2Name: "第2级指标节点名称",
      level3Code: "第3级指标节点代码",
      level3Name: "第3级指标节点名称",
      level4Code: "第4级指标节点代码",
      level4Name: "第4级指标节点名称",
      level5Code: "第5级指标节点代码",
      level5Name: "第5级指标节点名称",
      dataCode: "机构及产品指标编码",
      dataName: "机构及产品指标名称",
      isSummary: "是否汇总",
      isMinus: "是否减项",
  ...
  const handleDownloadTemplate = async () => {
    ...
    if (!proceed) return;
    try {
      await importWorkflow.downloadTemplate();
    } catch (e) {
  ...
  const handleFileSelect = async (file: File) => {
    ...
    try {
      const payload = await importWorkflow.preview(file);
  ...
  const handleImport = async () => {
    ...
    setIsLoading(true);
    try {
      const result = await importWorkflow.apply({
        file: selectedFile,
        mode: selectedMode,
        mappings: mappingObj,
      });
