...
import { treeToolbarButtonCompactClass } from "@/lib/shared/treeToolbarStyles";
...
function tokenizeFormulaExpression(expression: string): FormulaToken[] {
  ...
  while (i < expression.length) {
    ...
    if (/[A-Za-z]/.test(ch)) {
      ...
      tokens.push({ type: "identifier", value: expression.slice(i, end).toUpperCase() });
      i = end;
      continue;
    }
    if (/\d|\./.test(ch)) {
...
function evaluateFormulaExpression(expression: string): number {
  const tokens = tokenizeFormulaExpression(expression);
  let pos = 0;

  const peek = () => tokens[pos];
  const consume = () => tokens[pos++];
  const expectValue = (type: FormulaToken["type"], value?: string) => {
    const token = consume();
  ...
  const parseFunctionCall = (name: string): number => {
    expectValue("paren", "(");
    const args: number[] = [];
    ...
    expectValue("paren", ")");
    if (name === "SUM") return args.reduce((acc, cur) => acc + cur, 0);
    if (name === "AVG") return args.length === 0 ? 0 : args.reduce((acc, cur) => acc + cur, 0) / args.length;
  ...
  const parseFactor = (): number => {
    ...
    if (token.type === "identifier") {
      const name = String(consume().value);
    ...
    throw new Error(`公式语法错误：${token.value}`);
  };

  const result = parseExpression();
  if (pos < tokens.length) {
    throw new Error(`公式语法错误：${tokens[pos].value}`);
  }
...
type OrgProductMetricNodeDto = {
  ...
  name?: string;
  value_type?: string;
  allow_manual_entry?: number | string | boolean;
...
export function FormulaEditorDialog({
  ...
  /** 有搜索关键词时展开整棵科目树，便于命中节点可见（与机构及产品指标树选择行为一致） */
  useEffect(() => {
  ...
  const entityMatchesFormulaScope = (entityCode: string, productCode: string | null | undefined): boolean => {
    const owner = normalizeCode(entityCode);
    if (!owner) return false;
    if (currentAppliesToAllProducts) return owner === "AA";
    const currentProduct = normalizeCode(productCode);
    if (!currentProduct) return true;
    return owner === currentProduct || owner === "AA";
  };

  const buildSubjectTree = (
    productCode: string | null | undefined,
    orgProductSnapshot: OrgProductMetricSnapshotDto
  ): SubjectNode[] => {
    const roots: SubjectNode[] = [];
    for (const entity of orgProductSnapshot.entities ?? []) {
      const entityCode = normalizeCode(entity.entity_code);
      if (!entityMatchesFormulaScope(entityCode, productCode)) {
        continue;
      }
      const entityNode: SubjectNode = {
        id: `entity-${entityCode}`,
        code: entityCode,
        name: String(entity.entity_name || entityCode),
        type: "metric",
        isExpanded: true,
        children: [],
      };
      for (const table of entity.tables ?? []) {
        const tableName = String(table.table_name || "指标表");
        const tableNode: SubjectNode = {
          id: `table-${entityCode}-${tableName}`,
          code: entityCode,
          name: tableName,
          type: "metric",
          isExpanded: entityNode.children?.length === 0,
          children: [],
        };
        const walk = (metrics: OrgProductMetricNodeDto[], parent: SubjectNode) => {
          for (const metric of metrics) {
            ...
            const node: SubjectNode = {
              id: `metric-${entityCode}-${tableName}-${metricCode || dataAcctCode}`,
              code: metricCode || dataAcctCode || entityCode,
              name: String(metric.name || metricCode || dataAcctCode || "未命名指标"),
              type: "metric",
              isExpanded: false,
              children: [],
            };
            const childParent = node.children ? node : parent;
            if (metric.children?.length) walk(metric.children, childParent);
            if (dataAcctCode) {
              childParent.children = childParent.children ?? [];
              childParent.children.push({
                id: `data-${entityCode}-${tableName}-${metricCode}-${dataAcctCode}`,
                code: dataAcctCode,
                name: String(metric.name || dataAcctCode),
                type: "data",
                scopeLabel: entityCode === "AA" ? "全行" : "机构产品指标",
                sourceRef: `${entityCode}:${tableName}:${metricCode}`,
                appliesToAllProducts: entityCode === "AA",
              });
            }
        ...
        if (tableNode.children && tableNode.children.length > 0) {
          entityNode.children = entityNode.children ?? [];
          entityNode.children.push(tableNode);
        }
      }
    ...
    const pruneEmptyMetricNodes = (nodes: SubjectNode[]): SubjectNode[] =>
      nodes
        .map((node) => ({
          ...node,
          children: node.children ? pruneEmptyMetricNodes(node.children) : undefined,
        }))
        .filter((node) => node.type === "data" || (node.children && node.children.length > 0));

    const sortTree = (nodes: SubjectNode[]) => {
      nodes.sort((a, b) => a.code.localeCompare(b.code, "zh-CN"));
      nodes.forEach((n) => n.children && sortTree(n.children));
    };
    const prunedRoots = pruneEmptyMetricNodes(roots);
    sortTree(prunedRoots);
    return prunedRoots;
  };
  ...
  useEffect(() => {
    ...
    void (async () => {
      try {
        const orgProductSnapshot = await (getOrgProductMetricSnapshot() as Promise<OrgProductMetricSnapshotDto>).catch(() => ({ entities: [] }));
        if (cancelled) return;
        setTreeData(buildSubjectTree(currentProductCode, orgProductSnapshot));
        const validCodes = new Set<string>();
        const appliesToAllByCode = new Map<string, boolean>();
        for (const entity of orgProductSnapshot.entities ?? []) {
          const entityCode = normalizeCode(entity.entity_code);
          if (!entityMatchesFormulaScope(entityCode, currentProductCode)) continue;
          for (const table of entity.tables ?? []) {
            const walk = (metrics: OrgProductMetricNodeDto[]) => {
              for (const metric of metrics) {
                const dataAcctCode = deriveRuntimeRefFromOrgProductMetricCode(entityCode, metric.code);
                if (dataAcctCode) {
                  validCodes.add(dataAcctCode);
                  appliesToAllByCode.set(dataAcctCode, entityCode === "AA");
                }
        ...
        setAvailableDataCodes(validCodes);
        setDataAppliesToAllByCode(appliesToAllByCode);
      } catch (e) {
        if (cancelled) return;
  ...
  }, [isOpen, formulaType, currentProductCode, currentAppliesToAllProducts]);
  ...
  const validateFormula = (formulaToTest: string): { success: boolean; message?: string; result?: number; translatedFormula?: string } => {
    ...
    try {
      ...
      const subjectPattern = /<([A-Z0-9][A-Z0-9.]*[A-Z0-9])\s+[^>]+>/gi;
      let match;
      let replacementSeed = 1;

      while ((match = subjectPattern.exec(formulaToTest)) !== null) {
        const subjectCode = normalizeCode(match[1]);
        if (!availableDataCodes.has(subjectCode)) {
          return {
            success: false,
            message: `错误：科目代码 ${subjectCode} 不在当前机构产品指标体系中`
          };
        }
        if (currentAppliesToAllProducts && !dataAppliesToAllByCode.get(subjectCode)) {
          return {
            success: false,
            message: `错误：适用所有产品范围的公式仅可引用“适用所有产品范围”。当前引用 ${subjectCode} 不满足约束。`,
          };
        }
      ...
      const result = evaluateFormulaExpression(testFormula);
  ...
  const filterTree = (nodes: SubjectNode[], searchTerm: string): SubjectNode[] => {
    ...
    return nodes.filter(node => {
      const matchesCurrent =
        ...
        (node.sourceRef ?? "").toLowerCase().includes(searchLower);
    ...
    return (
      <div key={node.id}>
        <div
          ...
          title={node.sourceRef || `${node.code} ${node.name}`}
        >
          {hasChildren ? (
            node.isExpanded ? (
          ...
          <span className={`min-w-0 truncate ${node.type === 'data' && !dataNodeBlocked ? 'cursor-move' : ''}`}>{node.name}</span>

          {node.type === 'data' && (
            <span className="ml-auto flex-shrink-0 text-[10px] text-gray-400">
              {dataNodeBlocked
                ? "不在可引用范围"
                : `${node.scopeLabel || "机构产品指标"} · 可拖拽/双击`}
            </span>
          )}
        </div>
          ...
          <div className="w-[40%] border-r border-gray-200 flex flex-col">
            <div className="px-3 py-2 bg-gray-100 border-b border-gray-200">
              <div className="flex items-center justify-between mb-1">
                <h4 className="text-xs font-medium text-gray-700">机构产品指标</h4>
              ...
              <p className="text-[10px] text-gray-500 mb-2">
                {currentAppliesToAllProducts
                  ? "当前为适用所有产品范围：仅可引用“适用所有产品范围”（灰色项不可选）"
                  : "拖拽或双击已确认机构产品指标；公式引用直接使用同体系运行主键"}
              </p>
              <div className="relative">
                <Search className="w-3 h-3 absolute left-2 top-1/2 -translate-y-1/2 text-gray-400" />
                <input
                  type="text"
                  placeholder="搜索机构产品指标、运行编码或名称..."
                  value={searchText}
            ...
            <div className="flex-1 overflow-auto">
              {loadingTree ? (
                <div className="px-3 py-2 text-xs text-gray-500">正在加载机构产品指标...</div>
              ) : treeError ? (
                <div className="px-3 py-2 text-xs text-red-600">机构产品指标加载失败：{treeError}</div>
              ) : (
            ...
            <div className="flex-1 flex flex-col p-4">
              ...
              >
                ...
                <textarea
                  ...
                  placeholder="拖拽左侧机构产品指标到此处，或使用下方按钮编辑公式&#10;例如: <A01.01.01.01 产品利息收入_日均> * <A01.01.01.02 产品利息收入_收息率> / 12"
                ...
                >
                  {formula ? (
                    ...
                    <div className="text-gray-400 text-xs">
                      <p>拖拽左侧机构产品指标到此处，或使用下方按钮编辑公式</p>
                      <p className="mt-2">例如: <span className="text-pink-700 font-semibold">&lt;A01.01.01.01 产品利息收入_日均&gt;</span> <span className="text-green-600">*</span> <span className="text-pink-700 font-semibold">&lt;A01.01.01.02 产品利息收入_收息率&gt;</span> <span className="text-green-600">/</span> <span className="text-blue-600">12</span></p>
                    </div>
                  )}
                </div>
              </div>
