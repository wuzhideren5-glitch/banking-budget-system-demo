...
import { Header, type HeaderSession, type HeaderVersionSnapshotItem } from "./components/common/Header";
import { NavigationTree } from "./components/common/NavigationTree";
import { ChatBot } from "./components/system/ChatBot";
import { WorkArea, Tab } from "./components/common/WorkArea";
import { StatusBar, type StatusBarState } from "./components/common/StatusBar";
...
const OPEN_WORKSPACE_MODULE_EVENT = "budget-open-workspace-module";
...
export default function App() {
  ...
  const handleNavigationClick = useCallback((id: string, label: string) => {
    setTabs((prevTabs) => {
      if (prevTabs.some((tab) => tab.id === id)) return prevTabs;
      return [...prevTabs, { id, title: label }];
    });
    setActiveTab(id);
  }, []);
  ...
  useEffect(() => {
    const handler = (event: Event) => {
      ...
      if (!id || !label) return;
      handleNavigationClick(id, label);
    };
    window.addEventListener(OPEN_WORKSPACE_MODULE_EVENT, handler);
    return () => window.removeEventListener(OPEN_WORKSPACE_MODULE_EVENT, handler);
  ...
  if (!headerSession) {
    return (
      <div className="size-full flex items-center justify-center bg-[var(--bb-bg-app)]">
        <div className="bb-panel w-[360px] p-5">
          <h2 className="text-sm font-semibold text-[var(--bb-text-strong)] mb-4">预算系统登录</h2>
          <div className="space-y-3">
            ...
            <div className="relative">
              <input
                className="bb-input w-full pr-9 text-sm"
                placeholder="密码"
            ...
            <div className="text-[11px] text-[var(--bb-text-muted)] space-y-1">
              <p>1）用户名区分大小写，请按管理员分配的原始用户名输入。</p>
              <p>2）密码不少于8位，至少包含1个字母，且区分大小写。</p>
            </div>
            {authError && <div className="bb-status-banner bb-status-banner-danger">{authError}</div>}
            <button
              className="bb-btn bb-btn-primary w-full text-sm disabled:opacity-60"
  ...
  if (needChangePassword) {
    return (
      <div className="size-full flex items-center justify-center bg-[var(--bb-bg-app)]">
        <div className="bb-panel w-[420px] p-5">
          <h2 className="text-sm font-semibold text-[var(--bb-text-strong)] mb-2">首次登录请修改密码</h2>
          <p className="text-xs text-[var(--bb-text-muted)] mb-3">密码规则：至少8位，至少包含1个字母，且区分大小写。</p>
          <div className="relative">
            <input
              className="bb-input w-full pr-9 text-sm"
              placeholder="新密码"
          ...
          {authError && <div className="bb-status-banner bb-status-banner-danger mt-2">{authError}</div>}
          <button
            className="bb-btn bb-btn-primary w-full text-sm mt-3 disabled:opacity-60"
  ...
  return (
    ...
    <div className="bb-app-chrome size-full flex flex-col">
      <Header
      ...
      <div className="flex-1 overflow-hidden flex">
        {/* 左侧导航折叠按钮 */}
        {leftCollapsed && (
          <div className="w-8 bg-[var(--bb-bg-page)] border-r border-[var(--bb-border)] flex items-start justify-center pt-2 flex-shrink-0 z-[1]">
            <button
              type="button"
              onClick={() => setLeftCollapsed(false)}
              className="bb-icon-btn h-8 w-6 border border-[var(--bb-border)] bg-[var(--bb-bg-muted)]"
              title="展开导航"
            >
          ...
          <PanelGroup direction="horizontal">
            ...
            >
              ...
              <button
                ...
                className="bb-icon-btn absolute top-2 right-2 z-10 h-6 w-6 border border-[var(--bb-border)] bg-[var(--bb-bg-muted)]"
                title="折叠导航"
              >
            ...
            <PanelResizeHandle
              className={
                leftCollapsed
                  ...
                  : "w-1 bg-[var(--bb-border)] hover:bg-[var(--bb-primary)] transition-colors"
              }
            />
            ...
            <PanelResizeHandle
              className={`transition-colors ${
                rightCollapsed
                  ? "w-0 pointer-events-none"
                  : "w-1 bg-[var(--bb-border)] hover:bg-[var(--bb-primary)]"
              }`}
            />
            <Panel
        ...
        {/* 右侧聊天折叠按钮 */}
        {rightCollapsed && (
          <div className="w-8 bg-[var(--bb-bg-page)] border-l border-[var(--bb-border)] flex flex-col items-center pt-2 gap-1.5 flex-shrink-0 z-[1]">
            <button
              type="button"
