...
两页采用同一套表达：**四层业务主流程 + 一个底部数据沉淀与汇总支撑层**。主流程是 **数据来源层 -> 参数规则层 -> 数据处理层 -> 输出应用层**；底座承接事实表、汇总表、读模型和审计日志。这样可以把两个业务闭环分开讲，又避免把偏技术的数据表层误看成一个业务操作步骤。
...
| 图中位置 | 层级 | 业务含义 | 图中建议写法 |
| --- | --- | --- | --- |
| 主流程 | 数据来源层 | 外部报表、业务报送、人工录入、历史版本等原始输入 | “数据从哪里来” |
| 主流程 | 参数规则层 | 年度、版本、组织、科目、指标、映射、公式、规则等业务口径配置 | “按什么口径处理” |
| 主流程 | 数据处理层 | 导入、校验、匹配、写入、计算、汇总、刷新等系统动作 | “系统如何处理” |
| 主流程 | 输出应用层 | 报表、预测、专题、透视、智能报告、Agent 等最终使用场景 | “最后给谁用” |
| 底部支撑 | 数据沉淀与汇总底座 | 事实表、快照表、汇总表、读模型、审计日志等系统结果层 | “沉淀和支撑在哪里” |
...
```mermaid
flowchart LR
  X1["Excel报表导入：业务报送报表 / 管会P0301等报表"] --> X2["机构及产品数据录入"]
  X0["后期探索：管会系统对接（跨环境审批 / 合规评估）"] -.-> X2
  X2 --> X3["版本确认"]
  X3 --> X4["绑定到唯一指标号码"]
...
```mermaid
flowchart LR
  X1["Excel报表导入：BI系统报表 / 费用执行明细报表"] --> X2["费用执行明细导入"]
  X0["部门预算初始化导入"] --> X3["BI-AI 科目映射 / 归口映射"]
...
- 上方放四层主流程：数据来源 -> 参数规则 -> 数据处理 -> 输出应用。
- 底部横向放“数据沉淀与汇总底座”，承接事实表、汇总表、读模型和审计日志。
- 每层用 3 到 6 个关键词，避免塞满表名。
- 底部或右侧放一条工作顺序：配置 -> 录入 -> 确认 -> 写入 -> 计算 -> 输出。
...
- 复用第 1 页“四层主流程 + 数据底座”结构，让业务方一眼知道两页是同一套表达。
- 主流程突出“部门、科目、映射、预测、报表”。
- 底座突出费用实际明细、预测结果、人工覆盖、预算科目和报表读模型。
- 底部或右侧放一条工作顺序：主数据 -> 映射 -> 导入 -> 匹配 -> 预测 -> 报表。
