-- Extracted MySQL table initialization DDL
-- Source: deploy/mysql/banking_budget_20260623_full.sql.gz plus code-only lazy DDL for budget_annual_aggregate
-- Scope: current fresh/init table set; excludes migration backup/residual tables from the runtime dump.

CREATE DATABASE IF NOT EXISTS `banking_budget` DEFAULT CHARACTER SET utf8mb4 COLLATE utf8mb4_unicode_ci;
USE `banking_budget`;
...
-- Current views created by init/bootstrap
-- view: data_account
/*!50001 CREATE ALGORITHM=UNDEFINED */
/*!50013 DEFINER=`root`@`localhost` SQL SECURITY DEFINER */
/*!50001 VIEW `data_account` AS select `data_account_metric_node`.`node_code` AS `data_acct_code`,`data_account_metric_node`.`node_name` AS `data_acct_name`,`data_account_metric_node`.`budget_formula` AS `budget_formula`,`data_account_metric_node`.`actual_formula` AS `actual_formula`,`data_account_metric_node`.`budget_rule_code` AS `budget_rule_code`,`data_account_metric_node`.`budget_rule_config_json` AS `budget_rule_config_json`,`data_account_metric_node`.`need_calc` AS `need_calc`,`data_account_metric_node`.`formula_calc_mode` AS `formula_calc_mode`,`data_account_metric_node`.`allow_manual_entry` AS `allow_manual_entry`,`data_account_metric_node`.`value_type` AS `value_type`,`data_account_metric_node`.`remark` AS `remark` from `data_account_metric_node` where ((`data_account_metric_node`.`runtime_account_enabled` = 1) and (`data_account_metric_node`.`is_active` = 1)) */;

-- view: data_account_metric_binding
/*!50001 CREATE ALGORITHM=UNDEFINED */
/*!50013 DEFINER=`root`@`localhost` SQL SECURITY DEFINER */
/*!50001 VIEW `data_account_metric_binding` AS select `d`.`data_acct_code` AS `data_acct_code`,`n`.`node_code` AS `metric_node_code`,(case when (coalesce(`n`.`product_code`,'') = 'CORP') then 'CORP' else 'PRODUCT' end) AS `scope_type`,(case when (coalesce(`n`.`product_code`,'') <> '') then `n`.`product_code` when (locate('.',`n`.`node_code`) > 0) then substr(`n`.`node_code`,1,(locate('.',`n`.`node_code`) - 1)) else `n`.`node_code` end) AS `scope_code`,`n`.`sort_order` AS `sort_order`,`n`.`is_active` AS `is_active`,`n`.`remark` AS `remark`,`n`.`created_at` AS `created_at`,`n`.`updated_at` AS `updated_at` from (`data_account` `d` join `data_account_metric_node` `n` on((`n`.`node_code` = `d`.`data_acct_code`))) where (`n`.`is_active` = 1) */;
