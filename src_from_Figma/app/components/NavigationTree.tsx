...
const treeData: TreeNode[] = [
  ...
  },
  {
    ...
    children: [
      { id: "input-basic", label: "预算基础数据维护", icon: null },
    ],
  },
  {
    id: "analysis",
    ...
    children: [
      { id: "analysis-pivot-table", label: "数据透视表", icon: null },
      { id: "analysis-pivot-chart", label: "数据透视图", icon: null },
...
function TreeItem({ node, level = 0, onItemClick }: { node: TreeNode; level?: number; onItemClick?: (id: string, label: string) => void }) {
  ...
  return (
    <div>
      <div
        className="flex items-center gap-1.5 px-2 py-1.5 hover:bg-gray-100 cursor-pointer text-xs"
      ...
      >
        ...
        {node.icon && <span className="text-gray-600">{node.icon}</span>}
...
export function NavigationTree({ onItemClick }: { onItemClick?: (id: string, label: string) => void }) {
  return (
